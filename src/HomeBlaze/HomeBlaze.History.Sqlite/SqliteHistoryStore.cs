...
/// delegated to the graph-free <see cref="SqliteHistoryStoreCore"/>; this subject owns the change-queue
...
[InterceptorSubject]
public partial class SqliteHistoryStore : BackgroundService, IConfigurable, IHistoryStore, ILifecycleHandler
{
    ...
    private SqliteHistoryStoreCore? _core;

    public SqliteHistoryStore(ILogger<SqliteHistoryStore> logger)
    {
        _logger = logger;
    ...
    /// <summary>
    /// Test-only hook that forces the core to flush its pending samples immediately, so queued
    /// changes become queryable without waiting for the interval flush. Returns a completed task
    /// (no-op) before the core is built.
    /// </summary>
    internal Task FlushNowAsync(CancellationToken cancellationToken = default)
    {
        return _core?.FlushAsync(cancellationToken) ?? Task.CompletedTask;
    }
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        var core = new SqliteHistoryStoreCore(
            DatabasePath,
            PartitionInterval,
        ...
        // which changes would otherwise go unobserved. The core buffers samples and flushes on an interval,
        ...
        var flushTask = RunFlushLoopAsync(core, stoppingToken);
        try
        {
        ...
        finally
        {
            ...
            // cancelled here, so a fresh bounded token gives the flush a chance to complete (the core
            ...
            try
            {
                await core.FlushAsync(shutdownCancellation.Token).ConfigureAwait(false);
            }
    ...
    private ValueTask RecordBatch(
        SqliteHistoryStoreCore core, ISubjectPathResolver resolver, ReadOnlyMemory<SubjectPropertyChange> changes)
    {
    ...
    private async Task RunFlushLoopAsync(SqliteHistoryStoreCore core, CancellationToken stoppingToken)
    {
        try
        {
            while (!stoppingToken.IsCancellationRequested)
            {
                try
                {
                    await core.FlushAsync(stoppingToken).ConfigureAwait(false);
                    core.Sweep();
                }
                catch (OperationCanceledException) when (stoppingToken.IsCancellationRequested)
                {
                ...
                {
                    // The core keeps pending samples on a flush failure, so the next tick retries; the
    ...
    private void RefreshMetrics(SqliteHistoryStoreCore core)
    {
        RecordedCount = core.RecordedCount;
        OversizeCount = core.OversizeCount;
        QueueDepth = core.QueueDepth;
        EstimatedStorageBytes = core.EstimatedStorageBytes;
        LastFlushUtc = core.LastFlushUtc;
    ...
    public Task ApplyConfigurationAsync(CancellationToken cancellationToken)
    {
        ...
        // FlushInterval are read once when the core and change-queue processor are built in ExecuteAsync.
