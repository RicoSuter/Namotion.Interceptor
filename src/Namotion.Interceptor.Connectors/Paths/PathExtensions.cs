...
public static class PathExtensions
{
    ...
    public static bool UpdatePropertyValueFromSourcePath(this IInterceptorSubject subject, string sourcePath, DateTimeOffset timestamp, object? value, PathProviderBase pathProvider, object? source)
    {
        return subject
            .UpdatePropertyValueFromSourcePath(sourcePath, timestamp, (_, _) => value, pathProvider, source);
    }
    ...
    /// <param name="source">The optional source to mark the write as coming from this source to avoid updates.</param>
    /// <returns></returns>
    public static IEnumerable<string> UpdatePropertyValuesFromSourcePaths(this IInterceptorSubject subject, IEnumerable<string> sourcePaths, DateTimeOffset timestamp, Func<RegisteredSubjectProperty, string, object?> getPropertyValue, PathProviderBase pathProvider, object? source)
    {
        return subject
            .VisitPropertiesFromSourcePathsWithTimestamp(sourcePaths, timestamp, (property, path, _) => SetPropertyValue(property, timestamp, getPropertyValue(property, path), source), pathProvider);
    }
    ...
    public static IEnumerable<string> UpdatePropertyValuesFromSourcePaths(this IInterceptorSubject subject, IReadOnlyDictionary<string, object?> pathsAndValues, DateTimeOffset timestamp, PathProviderBase pathProvider, object? source)
    {
        return subject
            .VisitPropertiesFromSourcePathsWithTimestamp(pathsAndValues.Keys, timestamp, (property, path, _) => SetPropertyValue(property, timestamp, pathsAndValues[path], source), pathProvider);
    }
        ...
        PathProviderBase pathProvider, ISubjectFactory? subjectFactory = null)
    {
        ...
        {
            return VisitPropertiesFromSourcePaths(subject, sourcePaths, visitProperty, pathProvider, subjectFactory);
        }
    }
    ...
    public static string? TryGetSourcePath(this RegisteredSubjectProperty property, PathProviderBase pathProvider, IInterceptorSubject? rootSubject)
    {
        ...
        {
            return pathProvider.BuildPath(propertiesInPath);
        }
        ...
        this IEnumerable<RegisteredSubjectProperty> properties, PathProviderBase pathProvider, IInterceptorSubject? rootSubject)
    {
        ...
        {
            var path = property.TryGetSourcePath(pathProvider, rootSubject);
            if (path is not null)
            {
        ...
        this IEnumerable<SubjectPropertyChange> changes, PathProviderBase pathProvider, IInterceptorSubject? rootSubject)
    {
        foreach (var change in changes)
        {
            ...
            {
                var path = TryGetSourcePath(registeredProperty, pathProvider, rootSubject);
                if (path is not null)
                {
        ...
        PathProviderBase pathProvider, ISubjectFactory? subjectFactory = null)
    {
        ...
        foreach (var (path, property, index) in
            GetPropertiesFromSourcePaths(subject, sourcePaths, pathProvider, subjectFactory, useCache: false))
        {
    ...
    public static (RegisteredSubjectProperty? property, object? index) TryGetPropertyFromSourcePath(
        this IInterceptorSubject subject, string sourcePath, PathProviderBase pathProvider, ISubjectFactory? subjectFactory = null)
    {
        var (_, property, index) = subject
            .GetPropertiesFromSourcePaths([sourcePath], pathProvider, subjectFactory, useCache: false)
            .FirstOrDefault();
        ...
        bool useCache = true)
    {
        ...
        {
            var segments = pathProvider.ParsePath(sourcePath).ToArray();
