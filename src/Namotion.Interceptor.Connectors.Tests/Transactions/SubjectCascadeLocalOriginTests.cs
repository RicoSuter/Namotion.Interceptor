...
/// marked with a Local origin), and an INotifyPropertyChanged handler that writes back during a
/// source-originated apply publishes its write-back as local origin.
/// </summary>
public class SubjectCascadeLocalOriginTests : TransactionTestBase
{
    ...
    [Fact]
    public void WhenInpcHandlerWritesBackDuringSourceApply_ThenWriteBackPublishesLocalOrigin()
    {
        // Arrange
        var context = CreateContext();
        ...
        // Act: apply FirstName from a source, like an inbound apply. The one-shot origin stamps only
        // the FirstName write it targets; the INPC handler's LastName write-back is a nested write
        // that never consumes it, so the write-back publishes as local origin.
        new PropertyReference(person, nameof(Person.FirstName))
            .SetValueFromSource(sourceMock.Object, DateTimeOffset.UtcNow, null, "John");
        ...
        // Assert: the triggering FirstName write keeps the inbound source.
        ...
        Assert.Equal(ChangeOriginKind.Local, lastNameChanges[0].Origin.Kind);
    }
}
