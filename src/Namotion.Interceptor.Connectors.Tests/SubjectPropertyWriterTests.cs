...
public class SubjectPropertyWriterTests
{
    ...
    [Fact]
    public async Task WhenBufferExceedsMaxLimit_ThenOldestUpdatesAreDropped()
    {
        // Arrange
        var sourceMock = new Mock<ISubjectSource>();
        sourceMock
        ...
        const int maxBufferedUpdates = 5;
        var writer = new SubjectPropertyWriter(
            sourceMock.Object, null, NullLogger.Instance, maxBufferedUpdates);
        var updates = new List<int>();

        // Act - buffer more updates than the limit
        writer.StartBuffering();
        for (var i = 1; i <= 10; i++)
        {
            var value = i;
            writer.Write(updates, u => u.Add(value));
        }

        await writer.CompleteInitializationAsync(CancellationToken.None);

        // Assert - only the last 5 updates are replayed (oldest dropped)
        Assert.Equal(5, updates.Count);
        Assert.Equal([6, 7, 8, 9, 10], updates);
    }

    [Fact]
    public async Task WhenBufferExceedsMaxLimit_ThenWarningIsLogged()
    {
        // Arrange
        var sourceMock = new Mock<ISubjectSource>();
        sourceMock
        ...
        const int maxBufferedUpdates = 3;
        var writer = new SubjectPropertyWriter(
            sourceMock.Object, null, loggerMock.Object, maxBufferedUpdates);
        ...
        // Assert - warning was logged about dropped updates
        loggerMock.Verify(
            l => l.Log(
                LogLevel.Warning,
                It.IsAny<EventId>(),
                It.Is<It.IsAnyType>((v, t) => v.ToString()!.Contains("Dropped")),
                It.IsAny<Exception?>(),
    ...
    [Fact]
    public async Task WhenBufferAtExactLimit_ThenNoUpdatesAreDropped()
    {
        // Arrange
        var sourceMock = new Mock<ISubjectSource>();
        sourceMock
        ...
        // Assert - all updates replayed
        Assert.Equal(5, updates.Count);
        Assert.Equal([1, 2, 3, 4, 5], updates);
    }
}
