...
public class PathExtensionsTests
{
    ...
    {
        yield return ["default", DefaultConnectorPathProvider.Instance];
        yield return ["attribute", new AttributeBasedConnectorPathProvider("test", ".", null)];
    }

    [Theory]
    [MemberData(nameof(GetProviders))]
    public async Task WhenRetrievingAllPaths_ThenListIsCorrect(string name, IConnectorPathProvider connectorPathProvider)
    {
        // Arrange
    ...
    [Theory]
    [MemberData(nameof(GetProviders))]
    public async Task WhenApplyValuesFromSourceAndPaths_ThenSubjectAndChildrenShouldBeUpdated(string name, IConnectorPathProvider connectorPathProvider)
    {
        // Arrange
        var context = InterceptorSubjectContext
            .Create()
    ...
    public void WhenRetrievingPropertyPath_ThenItIsCorrect()
    {
        ...
        var path = person
            ...
            .TryGetSourcePath(DefaultConnectorPathProvider.Instance, null);
    ...
    public void WhenTryGetPropertyFromSourcePath_ReturnsResolvedProperty(string fullPath, string propertyName)
    {
        ...
        // Act
        var (property, _) = person.TryGetPropertyFromSourcePath(fullPath, DefaultConnectorPathProvider.Instance);
