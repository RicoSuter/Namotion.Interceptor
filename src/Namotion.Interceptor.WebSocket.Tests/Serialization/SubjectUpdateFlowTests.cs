...
public class SubjectUpdateFlowTests
{
    ...
    public void WelcomePayload_SerializeAndApply_CollectionWithItems_ShouldSyncCollectionIndexesCorrectly()
    {
        ...
        var welcome = new WelcomePayload { Version = WebSocketProtocol.Version, Format = WebSocketFormat.Json, State = update };
    ...
    public void WelcomePayload_SerializeAndApply_CollectionGrows_ShouldReuseExistingAndAddNew()
    {
        ...
        // Arrange - Client initialized via complete update (so stable IDs match the server's)
        var clientContext = InterceptorSubjectContext
            .Create()
            .WithFullPropertyTracking()
            .WithRegistry();
        ...
        var clientRoot = new TestRoot(clientContext);
        clientRoot.ApplySubjectUpdate(SubjectUpdate.CreateCompleteUpdate(serverRoot, []), DefaultSubjectFactory.Instance, ChangeOrigin.Local);

        // Capture references to the client items created by the initial complete update
        var existingItem0 = clientRoot.Items[0];
        var existingItem1 = clientRoot.Items[1];

        // Server grows: update existing items and add a new third item
    ...
    public void PartialUpdate_CollectionInsert_ShouldRoundTripThroughJson()
    {
        ...
        // Client initialized via complete update so stable IDs match the server's
