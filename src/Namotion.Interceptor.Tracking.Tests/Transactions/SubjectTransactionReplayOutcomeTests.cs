...
public class SubjectTransactionReplayOutcomeTests
{
    ...
    public async Task WhenReplayAcceptsUnchangedValue_ThenCommitSucceeds(bool withWriter, TransactionFailureHandling policy)
    {
        ...
        if (withWriter)
        {
            // A transforming hook can leave the source ahead of the model on an otherwise successful commit.
    ...
    public async Task WhenCompensationIsCanceled_ThenFailureIncludesOriginalChange(bool withWriter, TransactionFailureHandling policy)
    {
        ...
        var failedChange = Assert.Single(exception.FailedChanges);
    ...
    private static ReplayOutcomeSubject[] CreateSubjects()
    {
        // A subject without a context assigns its field directly and never enters the write pipeline,
        // so it cannot report an outcome; a transaction cannot capture such a subject either.
        var context = InterceptorSubjectContext.Create();
        return Enumerable.Range(0, 512).Select(_ => new ReplayOutcomeSubject(context) { Value = "old" }).ToArray();
    }
