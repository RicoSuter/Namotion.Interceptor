...
public class PropertyChangeOldValueTests
{
    ...
    [Fact]
    public void WhenASettableDerivedPropertyIsWritten_ThenItsRecalculationPublishesThePreviousResultAsOldValue()
    {
        // Arrange
        var context = InterceptorSubjectContext.Create().WithFullPropertyTracking();
        ...
        person.Nickname = "second";

        // Assert: each write publishes twice, once for the setter's commit and once for the
        // recalculation it triggers. The recalculation supplies the previous result as the old value
        // itself; by then the store already holds the new value, which must not replace it.
        var nicknameChanges = changes
            .Where(change => change.Property.Name == nameof(DerivedSetterPerson.Nickname))
            .ToList();

        Assert.Equal(4, nicknameChanges.Count);
        Assert.All(nicknameChanges.Where(change => change.GetNewValue<string?>() == "first"),
            change => Assert.Null(change.GetOldValue<string?>()));
        Assert.All(nicknameChanges.Where(change => change.GetNewValue<string?>() == "second"),
            change => Assert.Equal("first", change.GetOldValue<string?>()));
    }
