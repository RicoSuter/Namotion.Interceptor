...
public class DerivedPropertyChangeHandler : IReadInterceptor, IWriteInterceptor, IPropertyLifecycleHandler
{
        ...
        long rawTimestamp)
    {
        ...
        // oldValue by the guards above, so publishing must reuse it rather than re-invoke the
        // getter, which could return a later value that never coexisted with oldValue.
        derivedProperty.SetPropertyValueWithInterception(
            newValue, oldValue, NoOpWriteDelegate, rawTimestamp, finalValueIsNewValue: true);
