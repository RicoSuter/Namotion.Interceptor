...
public static class LifecycleInterceptorExtensions
{
    ...
    {
        if (subject.TryGetContext()?.TryGetLifecycleInterceptor()?.TryQueuePropertyCallback(property, attach: true) == true)
        {
            return;
        }
    ...
    {
        if (subject.TryGetContext()?.TryGetLifecycleInterceptor()?.TryQueuePropertyCallback(property, attach: false) == true)
        {
            return;
        }
