...
/// <remarks>
/// The intrusive chain and the per-subject last-occurrence map exist so <see cref="RemoveLast"/> is
/// O(1). A plain list cannot reach that: removing by index shifts every later occurrence, which
/// invalidates any index map, and scanning for the trailing occurrence instead turns a reconcile
/// that drops a prefix of a large collection into quadratic work. Measured on a 20,000 child
/// collection whose leading half is replaced, a scanning list costs 136 ms against 22 ms here, and
/// the gap widens with size. Keep the O(1) removal if this is ever rewritten.
/// </remarks>
