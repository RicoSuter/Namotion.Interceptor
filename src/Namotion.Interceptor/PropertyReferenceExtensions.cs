...
public static class PropertyReferenceExtensions
{
    ...
    /// dance the derived-cascade would otherwise need to share the trigger's snapped time.
    /// </summary>
    internal static void SetPropertyValueWithInterception(this PropertyReference property, object? newValue,
        object? currentValue, Action<IInterceptorSubject, object?> writeValue, long preResolvedRawTimestamp)
    {
        var executor = property.Subject.Context as InterceptorExecutor;
        executor?.SetPropertyValue(property.Name, newValue, currentValue, writeValue, preResolvedRawTimestamp);
    }
}
