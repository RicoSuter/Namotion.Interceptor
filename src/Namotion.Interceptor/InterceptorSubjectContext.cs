...
public class InterceptorSubjectContext : IInterceptorSubjectContext
{
    ...
    // Ownership records for fallback edges added through InterceptorExecutor, so a detach notifies
    // exactly what the matching attach resolved. Null on every other context. Read and written
    // only under _mutationLock, which is what makes a record atomic with the edge it owns, and
    // never touched by a resolution or invalidation path.
    ...
    /// <summary>
    /// Ownership record for one fallback edge added through <see cref="Interceptors.InterceptorExecutor"/>.
    /// Every field is read and written under <c>_mutationLock</c> only.
    /// </summary>
    ...
    private protected enum FallbackRemovalOutcome
    {
        /// <summary>No edge, or another remover already owns it.</summary>
        NotPresent,

        /// <summary>Handed to the thread still attaching; it will run the callbacks and the removal.</summary>
        Deferred,
    ...
    /// <summary>
    /// Publishes the edge and its ownership record in one locked section, so no observer that
    /// takes the lock can see an edge without its record. Returns null when the edge exists.
    /// </summary>
    ...
    {
        // Allocated before the lock so an allocation failure cannot leave the edge published
        // without a record to remove it by.
        ...
        lock (_mutationLock)
        {
            ...
            var newState = new ContextState(state.Services, state.FallbackContexts.Add(contextImpl));

            // R4, as in AddFallbackContext: register into the fallback before publishing.
            var usedByContexts = contextImpl.GetOrCreateUsedByContexts();
            lock (usedByContexts)
            {
                usedByContexts.Add(this);
        ...
        catch
        {
            ...
            // permanently unremovable, since every later remove defers to an attach that will
            // never complete.
    ...
    /// <summary>
    /// Marks the attach finished and reports whether a remover deferred its removal to this
    /// thread. Must be called from a finally, so a throwing attach still leaves a removable edge.
    /// </summary>
    ...
    /// <summary>
    /// Phase one of removal. Claims the record and leaves the edge, because the detach callbacks
    /// resolve their handlers through it. Publishes nothing, so no invalidation and no cache loss.
    /// </summary>
        ...
        out FallbackAttachment? attachment)
    {
        lock (_mutationLock)
        {
            ...
            {
                // Waiting here would deadlock: the attaching thread is inside callbacks that take
                // the lifecycle lock, which this caller may already hold. Hand the removal over
                // instead, so the edge is never left behind.
                if (attachment.IsPendingRemoval)
                {
                    attachment = null;
                    return FallbackRemovalOutcome.NotPresent;
                ...
                attachment = null;
                return FallbackRemovalOutcome.Deferred;
            }
    ...
    private protected void CompleteFallbackContextRemoval(InterceptorSubjectContext contextImpl)
    {
        lock (_mutationLock)
        {
            ...
            // R4, as in RemoveFallbackContext: unregister only after publishing.
            var usedByContexts = Volatile.Read(ref contextImpl._usedByContexts);
            if (usedByContexts is not null)
            {
                lock (usedByContexts)
