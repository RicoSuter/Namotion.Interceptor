...
2. **Validate.** Deduplicate and resolve deterministic address-space conflicts as the plan is built (this runs within discovery rather than as a separate pass): duplicate dynamic property or attribute names, duplicate structured targets, duplicate dictionary keys, and multiple browse paths to the same value property (resolved by keeping the smaller node id). A container whose browse failed this round is left untouched so existing items are preserved rather than overwritten with an empty collection.
