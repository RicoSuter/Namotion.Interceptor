...
When an implemented `OnXChanging` hook transforms the incoming value (reassigns `ref newValue`) during a source-scoped write, the property write itself also publishes as local origin: the setter compares the hook's output against the incoming value (`EqualityComparer<T>.Default`) and enters the local-origin scope for the write when they differ. The stored value was computed locally, so it flows back to a bound source instead of being suppressed as an echo of that source. Transforms must be projections (idempotent, like clamping or rounding) so the correction converges; reference-typed values must be reassigned, not mutated in place, for the transform to be detected. The comparison runs only when an ambient source is set, so plain local writes never pay for it (their origin is already local); properties without an implemented `OnXChanging` hook skip it entirely.

Generated setters call `RaisePropertyChanged` directly. Both generated and hand-written `IRaisePropertyChanged` implementations follow one contract: check subscribers first (an unused event costs only a null check), then invoke handlers under `SubjectChangeContext.WithLocalOrigin()` when an ambient source is set. The `SubjectChangeContext.RaisePropertyChanged(handler, sender, propertyName)` helper implements the contract in one call; generated raisers forward to it, and hand-written implementations should do the same. When a manual implementation is explicit and therefore has no directly callable method, the generator emits a protected forwarding method for generated setters and descendants.
