...
- `SetPropertyValue` is also accepted in the shape generator versions up to 0.9.3 emitted, `(string propertyName, TProperty newValue, TProperty currentValue, Action<IInterceptorSubject, TProperty> setValue)`, so a base assembly those versions built keeps hosting subjects without a rebuild. The generated setters then pass the field instead of a delegate, and the properties of the whole hierarchy keep that version's old value semantics: a change's old value is the value the setter read before the write started, which a concurrent writer can supersede before the write commits. Declare the delegate shape, or rebuild the base assembly, to get the exact old value described in [Delivery Guarantees](tracking.md#delivery-guarantees).
