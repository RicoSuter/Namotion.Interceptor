...
Generated setters call `RaisePropertyChanged` directly. For generated events, the method first checks whether the event has subscribers and returns before reading the ambient source, preserving the unused-INPC fast path. A hand-written `IRaisePropertyChanged` implementation is responsible for invoking its handlers under `SubjectChangeContext.WithLocalOrigin()` and preserving the same subscriber-first fast path. When the manual implementation is explicit and therefore has no directly callable method, the generator emits a protected forwarding method for generated setters and descendants.
