...
## Why parked writes are flushed while idle, not only on the next change
...
`SubjectSourceBase` therefore runs an idle flush for the life of the connected phase, alongside the processor rather than replacing it: it flushes the retry queue every `_retryTime` and each time the source becomes `Synchronized`. It skips the flush while the gate above is held, so it never races the reconcile, and while the source is not `Synchronized`, which after a reconnect whose load failed lasts until the next reconnect and leaves parked writes to the next local write; an empty queue never reaches the source. Every connector inherits this the same way it inherits the gate, because both live in `SubjectSourceBase` rather than in a connector-specific reconnect path.
