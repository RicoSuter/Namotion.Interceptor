...
It marks the target detached, appends a stop, then **releases** rather than only retiring. The release is the opposite of what the explicit detach above does, and it is safe for a different reason than that one: the removal has put the target out of reach, so an ownership left installed is never retired, and the stop appended ahead of the release reads `Current` and never `Owner`, so the order between the two stops mattering. The stop is there because a start queued against this target while the awaited one was in flight is not ruled out, which [Faults and Failed Starts](#faults-and-failed-starts) works through. The mark goes first, so a context attach that snapshotted the attachment before the removal cannot take the target in the gap. Pinned by `HostedServiceHandlerTests.WhenAnAwaitedAttachFaults_ThenTheHandlerStopsRetainingItsTarget` for the release and `HostedServiceHandlerRaceTests.WhenAFaultedAwaitedAttachReleasesATargetWhoseQueuedStartIsCommitted_ThenThatStartsInstanceIsStoppedAndDisposed` for the stop; the mark has no test, for the reason the other two marks do not.
...
The flag read is discriminated, by the explicit detach above, which retires the ownership record without releasing it, so the body still reads this handler as the owner and only the flag refuses it. The ownership read alone is not discriminated: deleting it leaves the suite green. That is a coverage gap rather than redundancy.

One shape that reaches it is settled. A context detach clears liveness and releases, an explicit detach of one of two attachments then keeps the re-attach from re-taking that target, and the context attach behind it writes liveness again, leaving a queued body live and unowned. It needs no race and no second interceptor, and with the ownership read deleted the body creates an instance the queued stops then stop and dispose, so the cost is bounded.
...
- **The ownership read inside the start body, taken alone.** Deleting it leaves the suite green while deleting the flag read beside it fails a test, so this is the half of the pair with nothing behind it. It is not an equivalent mutant: [The read inside the start body](#the-read-inside-the-start-body) gives a race free shape that reaches it, and a test for that one is worth writing. The second shape recorded there is a race, and what pins it is the stop appended behind it rather than a test of the refusal, which would pin an ordering production does not provide.
