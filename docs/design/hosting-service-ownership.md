...
_chainLock            Lock                     guards _tail and _detached
_ownershipLock        Lock                     guards the owner exchange and the handler's record of it; the take and its own undo run under _chainLock, every other release does not, deliberately so for the context detach
...
- `_snapshot` and `_fault` use `Volatile.Read` and `Volatile.Write`, and both are written by transition bodies only, which the chain serializes. Awaiting a transition already gives its awaiter a happens before edge, but a diagnostics poll reads `Current` from an unrelated thread with no such edge, so this is required rather than decorative. `_snapshot` carries the instance and the transition phase together because a consumer reads them as one fact, and nothing else branches on the phase; holding the pair in one reference field is what makes every transition a single write and what takes the pair back out in a single load (see [The state a consumer polls](#the-state-a-consumer-polls)).
...
- `_detached` is written and read under `_chainLock`, which is what pairs it with the append (see [Refusing a start for an attachment a detach already removed](#refusing-a-start-for-an-attachment-a-detach-already-removed)). `GetState` is the one read outside it, deliberately, so polling the handle never queues behind an append.
...
The logger is assigned after construction rather than injected because `WithHostedServices` constructs the handler while the context is being configured, which is before any service provider exists; the registration it adds to the `IServiceCollection` calls `SetLogger` when the provider builds it.
...
Ordering holds because both appends happen under the lifecycle lock, so any later re-attach queues behind them on the same chains. The wait is acyclic: an attachment chain waits on the subject's signal and the subject's chain waits on nothing, **provided the subject's stop does not itself wait on an attachment chain**. That proviso is [deadlock shape 3](#3-a-subject-that-detaches-its-own-attachment-while-unwinding).
...
It marks the target detached, appends a stop, then **releases** rather than only retiring. It releases because the removal has put the target out of reach, so an ownership left installed is retired by nothing before the drain. It is safe to release ahead of the stop because that stop's body reads `Current` and never `Owner`, so the order between the two does not matter. The stop is there because a start queued against this target while the awaited one was in flight is not ruled out, which [Faults and Failed Starts](#faults-and-failed-starts) works through. The mark goes first, so a context attach that snapshotted the attachment before the removal cannot take the target in the gap. Pinned by `HostedServiceHandlerTests.WhenAnAwaitedAttachFaults_ThenTheHandlerStopsRetainingItsTarget` for the release and `HostedServiceHandlerRaceTests.WhenAFaultedAwaitedAttachReleasesATargetWhoseQueuedStartIsCommitted_ThenThatStartsInstanceIsStoppedAndDisposed` for the stop; the mark has no test, for the reason the other two marks do not.
...
The flag read is discriminated, by the explicit detach above, which retires the ownership record without releasing it, so the body still reads this handler as the owner and only the flag refuses it. The ownership read is discriminated the other way round, by `HostedServiceHandlerTests.WhenAQueuedStartsSubjectIsLiveAgainButItsOwnershipWasNotRetaken_ThenNothingIsCreated`, which fails when that read alone is deleted.

One shape reaches it, and that is the shape the test above drives. A context detach clears liveness and releases, an explicit detach of one of two attachments then keeps the re-attach from re-taking that target, and the context attach behind it writes liveness again, leaving a queued body live and unowned. It needs no race and no second interceptor: the host is built and never started, so the queued body parks at the startup gate and starting the host releases it into the guard. With the ownership read deleted the body creates an instance the queued stops then stop and dispose, so the cost is bounded. The test pins that one shape rather than every way a body can outlive its ownership.

Every attempt to write down a second shape has been wrong under review, so what stands here is only what was measured. A start can be queued behind the one a faulted awaited attach releases for, because the attachment is published before that start is awaited. Reaching it needs a second `LifecycleInterceptor`, since one interceptor dispatches an attach only once for a subject already in its graph. What that queued start meets is not the ownership read but a race with the caller's own release, which the stop that path appends resolves without ordering them. That is [Faults and Failed Starts](#faults-and-failed-starts) rather than a gap here.
...
A start whose factory returns the instance it returned last time is refused before `StartAsync` and before the instance is recorded, with an `InvalidOperationException` recorded on `Fault` and `Current` left null. The rule and what it does not catch are stated for consumers in [The factory must construct](../hosting.md#the-factory-must-construct).
...
**A faulted awaited attach appends a stop, because a start queued behind it cannot be ordered against its cleanup.** `AttachHostedServiceAsync` publishes the attachment before it awaits the start, so a second start can be appended against the same target while the first is in flight. When the awaited start faults, the same completion releases both the caller's cleanup and that queued body, and nothing orders the two: the body's ownership read refuses it only if the release lands first. Before the stop was appended, a probe that forced the queue measured the body winning between a quarter and a half of the time, each iteration leaving an instance started, never stopped and never disposed after the host had stopped, with every other guard in place.

The fix is not to order them but to stop depending on the order. The cleanup marks the target detached, appends a stop and then releases. `AppendStop` reads no `_detached`, so the mark does not refuse it; the chain is first in first out, so it lands behind a start already appended and stops and disposes whatever that start created; the stop body reads `Current` and never ownership, so the release behind it changes nothing; and the mark refuses every later start, which is the only append that could put an instance behind the stop. It is not awaited, because the queued start may be parked on a startup scope this caller cannot close, which is [deadlock shape 5](#5-awaiting-a-captured-start-or-its-detach-inside-its-own-startup-scope). The append is attributed, so shutdown waits for it. Pinned by `HostedServiceHandlerRaceTests.WhenAFaultedAwaitedAttachReleasesATargetWhoseQueuedStartIsCommitted_ThenThatStartsInstanceIsStoppedAndDisposed`, which asserts the ordering the chain provides rather than the refusal it does not: deleting the append fails it deterministically, measured six times including under two cores.

The queued start needs a second `LifecycleInterceptor` over the subject, because one interceptor raises a context attach only once for a subject already in its graph, so it shares the precondition with [A handler can be marked live for a graph it does not serve](#a-handler-can-be-marked-live-for-a-graph-it-does-not-serve). The cleanup around it is new either way: master performs none on this path and leaks the attachment entry on every failed attach instead, which is what the cleanup exists to fix.
...
Those two members leave four situations indistinguishable: nothing has started yet, a start is in flight, a re-attach disposed the previous instance and has not created the next one, and a start was refused. `HostedServiceAttachmentState` separates a start in flight, a stop in flight and a terminal attachment from the rest, and `HostedServiceTarget.State` derives it from the snapshot the transitions already write rather than storing a field of its own, so `Running` is structurally identical to that snapshot holding an instance and can never disagree with `Current`. It does not separate the first situation from the third once the re-attach's stop has settled: nothing on the handle carries history, so both read `Stopped`, and what the state adds on that path is the window itself, `Starting` while the next instance is being created and `Stopping` while the previous one is going away. It separates the fourth only when the reason was a detach: a closed gate, a subject that is no longer live, ownership that has moved on and a target that already holds an instance all return before the start enters its window and before it clears the fault, so each leaves the target reading exactly what it read before, whatever that was. There is no state a refusal puts a target into, and no set of states a refused target is confined to: the one instance guard refuses precisely because `Current` is non null, so a refusal can leave a target reading `Running`. `Stopped` is therefore "nothing is running", and neither "a start would be accepted" nor "nothing is queued": a start parked on a startup scope waits inside its own body, below the guards and above the window, and reads `Stopped` for the whole deferral unless the target already carries a fault or a detach mark, which read `Faulted` and `Removed`, because the scope wait sits above the fault clear.

**The instance and the phase are one field.** `HostedServiceTarget` holds them as an immutable `(instance, phase)` pair in a single reference field, and every transition changes that pair with at most one `Volatile.Write`. A start enters `(null, Starting)` past every guard and immediately before the factory, so no refused start reports `Starting`, and it then records `(instance, None)`, which is the same write that leaves the start window. A stop enters `(null, Stopping)` with the write that takes the instance out of `Current`, so the target never holds no instance while raising no phase, which is the state a poll must never be shown: it reads as settled while the instance still holds its sessions, semaphores and subscriptions. Each `finally` settles back to `(null, None)` only while its own phase is still the one in flight, and the two conditions are not load bearing to the same degree. The start's is: `CompleteStart` has already replaced the pair by the time it runs, so an unconditional settle there would null an instance that has just started. The stop's is not, for the body as it stands: the one exit above the phase change is the return taken because the target held no instance, nothing else writes the pair while that body runs, and an unconditional settle would therefore write the value already in the field. It is conditional so that `EndStop` is correct on its own rather than on what precedes it, because a guard added above the instance read later would otherwise turn it into a settle over a live instance and nothing would say so. Both are plain read modify writes rather than compare and exchanges, because the chain serializes the bodies for one target. The three pairs that hold no instance are cached statics, so a settled target allocates nothing and only recording an instance allocates at all.

**What makes these readings consistent is the shape of those writes, and no test pins that.** `GetState` and `Current` each take the instance and the phase in one load, so there is no read between those two for a transition to land in and no order to get wrong between them.

`GetState` is not one load in total. It takes the snapshot, and where that comes back settled it goes on to read `_detached` and then `Fault`, which is three loads on the path that reaches `Stopped`. Those two later loads are sound rather than exempt: they run only once the snapshot has come back holding no instance and raising no phase, and between them they can only choose among `Removed`, `Faulted` and `Stopped`. None of those three claims a transition is in flight, which is the claim a stale reading would have to make to be dangerous, so the worst either one does is report a settled target one moment older than it is.

The earlier design was the one that had a read order inside it. It held the instance and two flags in three fields, which forced a specific and asymmetric order on the derivation: the start flag before the instance, the stop flag after it, each pairing an acquire with the release the body it belongs to had already made. That order was got wrong three times by three authors, in the version that introduced it, in the fix prescribed for that, and in a third gap measured afterwards where moving the stop flag's load above the instance left the entire suite green. Covering a window of that kind at all needs a seam inside the method, holding one of the reads open while a transition runs between it and the next. The pair is one load now, so there is nothing left for such a seam to sit between.

**The read order was removed rather than moved.** `IHostedServiceAttachment<T>.GetState(out T? current)` hands the state and the instance out of one snapshot load, so a consumer cannot hold a state that disagrees with the instance it then acts on. `HomeBlaze.OpcUa.SingleAttachmentHost.UpdateFromAttachment` takes one such reading per branch, immediately before the drop it decides. One ordering rule survives and is not pinned: the fault is read before either reading, because a retry start clears the fault before it enters its start window, so a reading taken above the fault would still be settled for a fault this poll is about to act on.

Nothing in the suite stands in for the single write rule, and nothing can. An edit that split a transition back into two writes, clearing the instance and then raising the stop phase as two statements for example, would reintroduce the original bug with the suite still green: between the two statements the pair holds no instance and raises no phase, which is the settled reading a poll must never be shown. Taken the other way round the intermediate pair still holds the instance and reads `Running`, which is harmless, so the rule cannot be an order to remember. It is therefore stated on the transition methods themselves as a shape: each has to stay one write.

What the suite does pin is that the transitions exist and sit where they do. Each of these mutations was run against the whole non integration suite, and each fails at least one test. Count the assembly summaries when repeating one: a project that fails stops the rest of them being scheduled, so a run that reports fewer than the full set has not measured the mutation, it has measured part of it.

- the start's entry moved above the four guards in `RunStartAsync`: 6 failures, headed by `HostedServiceHandlerTests.WhenAParkedStartIsRefusedAfterItsSubjectLeftTheGraph_ThenTheAttachmentReportsStopped`;
- the start's `finally` deleted, so a failed start never leaves the start window: 3 failures, `HostedServiceHandlerTests.WhenAnAwaitedAttachFaults_ThenTheAttachmentReportsRemoved`, `HostedServiceHandlerTests.WhenATransitionFaultedEarlier_ThenTheNextSuccessfulOneClearsTheFault` and `HomeBlaze.OpcUa.Tests.OpcUaClientTests.WhenTheInstanceIsReplacedByAFault_ThenTheDiagnosticsAndTheRootAreCleared`;
- the start's `finally` made an unconditional settle, so a successful start nulls the instance it has just recorded: 69 failures, 55 of them in the hosting suite;
- the stop's entry deleted, so the instance never leaves `Current`: 20 failures, including `HostedServiceHandlerTests.WhenADetachIsHeldInsideTheInstanceStop_ThenTheAttachmentReportsStopping`;
- the `EndStop` call deleted, so a stopped target never leaves the stop window: 2 failures, `HostedServiceHandlerTests.WhenADetachIsHeldInsideTheInstanceStop_ThenTheAttachmentReportsStopping` and `HostedServiceHandlerTests.WhenAStartedAttachmentIsDetached_ThenItLeavesTheStartWindowBehind`;
- `Faulted` ranked above `Removed`: `HostedServiceHandlerTests.WhenAnAwaitedAttachFaults_ThenTheAttachmentReportsRemoved`;
- `Removed` ranked above `Stopping`: `HostedServiceHandlerTests.WhenADetachIsHeldInsideTheInstanceStop_ThenTheAttachmentReportsStopping`;
- the `Faulted` arm deleted from `GetState`, so a target carrying a fault settles to `Stopped`: `HostedServiceHandlerTests.WhenATransitionFaultedEarlier_ThenTheNextSuccessfulOneClearsTheFault`, which fails on the assertion rather than by running out of a wait;
- the drop guard on the not running branch of `HomeBlaze.OpcUa.SingleAttachmentHost.UpdateFromAttachment` removed: `OpcUaClientTests.WhenAReconciliationLandsInsideAReAttach_ThenTheReAttachStillProducesARunningClient`. The drop itself is pinned the other way by `OpcUaClientTests.WhenTheAttachmentHoldsNoInstance_ThenTheReconciliationDropsTheRoot`, so that branch is held from both sides.

What the list does not reach on the stop's `finally` is its condition. The call itself is pinned, as the bullet above records; made an unconditional settle it leaves the whole suite green, which is the measurement behind the remarks on `EndStop`: for the stop body as it stands the condition decides nothing, and it is there so that a guard added above the instance read later cannot quietly turn the settle into one over a live instance.

The two ranking decisions in the precedence are argued on the property itself. `Faulted` is deliberately not terminal, because a start clears the fault after its guards and before the factory, so the next attach can retry; `Removed` is the terminal one, and the distinction between the two is what a consumer acts on. `Removed` is not "nothing runs after this point": a start appended before the mark still runs, because no guard in `RunStartAsync` reads `_detached`, and the stop the detach appends behind it disposes what that start created. What the mark is terminal for is every start appended after it.
...
2. Clear `_liveSubjects`, for the reason recorded there, and because it is also what stops `WaitForStartAsync` appending an empty transition behind the drain's own stop, which is what `HostedServiceHandlerTests.WhenAHandlerIsAskedToWaitWhileItsOwnDrainIsStopping_ThenItAnswersWithoutQueueingBehindTheStop` pins: deleting the clear fails that test and `HostedServiceHandlerTests.WhenAnAttachmentIsAddedAfterTheDrainClearedLiveness_ThenTheSubjectIsNotLeftLive`, which waits on liveness going false and so observes the clear's effect rather than the statement. That one fails at the wait rather than at an assertion, so it reports the mutation as a timeout rather than as the property it went on to check.
3. Snapshot `_owned`.
...
A subject that hosts nothing never gets a target and never reaches `AppendCore`, so it pays nothing at all. On the detach path the change is cheaper: one increment and one decrement replace a per stop continuation and a dictionary entry. The unit suite's hosting project runs in the same 10 seconds of wall clock it did before, about 7 of them inside the tests, and both shutdown deadline tests are unchanged.
...
Per target chains contain the self deadlock that one shared queue spread across every service. They do not remove cycles, and none of the five shapes below is detected at runtime.

Every one of them is a constraint on consumer code rather than a defect here, so each is stated rather than fixed. Detecting them would mean tracking waits across chains, which is not worth its cost against a rule that can be written down, and shape 3, the only one that occurs in practice, is complied with in tree and pinned by a regression test. Shape 5 is the exception in kind: it is an accepted limitation rather than a rule a consumer can simply follow, because the host reaches it without the consumer writing anything unusual, and `HostOptions.StartupTimeout` is what turns the hang into a failure.

A sixth shape is not here because it is not a consumer shape: [a handler that loses the exchange to a departing handler never retries](#a-handler-that-loses-the-exchange-to-a-departing-handler-never-retries) is an internal window and a known defect.
...
- **Every transition on `HostedServiceTarget` staying a single write.** This is the one entry that is not a guard but a shape, and it is what [the state a consumer polls](#the-state-a-consumer-polls) rests on entirely. Splitting `BeginStop` into a clear of the instance followed by a raise of the phase leaves the suite green while reopening the window a poll reads a settled state through, and no seam can catch it: the pair is one load now, so there is nothing for a seam to sit between, and a seam in a transition body would drive the writer while the reader is the side that has to observe the gap.
- **The drop guard on the fault branch of `HomeBlaze.OpcUa.SingleAttachmentHost.UpdateFromAttachment`.** Removing it leaves the suite green. The branch it protects needs a retry start to clear the fault and publish its tree between this poll's fault read and its drop, and the suite only ever reaches that branch with the attachment settled, which `OpcUaClientTests.WhenTheInstanceIsReplacedByAFault_ThenTheDiagnosticsAndTheRootAreCleared` covers. The guard on the other branch is pinned, by `OpcUaClientTests.WhenAReconciliationLandsInsideAReAttach_ThenTheReAttachStillProducesARunningClient`.
- **That `GetState` takes one snapshot load.** Taking the instance from a second, independent load of the same field leaves the suite green, and no test can do better without a seam between the two, which is the surface the three load design was replaced to remove. The method exists so a caller cannot hold a state that disagrees with the instance it acts on, and that guarantee rests on the single load rather than on a test. It is the same kind of entry as the one write per transition rule above.
- **That a drop takes its reading immediately before dropping.** Hoisting either reading to the top of `UpdateFromAttachment` leaves the suite green, and it is what reopens the window: an intercepted property write then sits between the reading and the drop it decides, which is long enough for a start to publish a whole tree into it. The combined reading removes the state against instance mismatch, not this.
- **Treating `Stopping` as settled.** Holding a drop back for a stop in flight as well leaves the suite green. Excluding it is a decision rather than an oversight: an instance that has left `Current` is being stopped and disposed, so what it published is the wrapper's to drop, where a start in flight may be publishing into the same tree the drop would clear.
- **That the fault is read before the state.** Swapping them leaves the suite green. A retry start clears the fault before it enters its start window, so a reading taken above the fault would still be settled for a fault this poll is about to act on.
- **The `Volatile` qualifiers on `_snapshot`, `_fault` and `_detached`.** Dropping any of them to plain field access leaves the suite green, and a longer run on x64 would be a poor test of them: the loads and stores they pair are already ordered by that hardware, so what is left to go wrong is the JIT keeping a polled read in a register rather than taking it again, and the ordering on a weaker memory model. They are what makes a poll on an unrelated thread, which has no happens before edge to the transition body that wrote the field, a defined read rather than a race. `_detached` carries one because of where `GetState` reads it: every other read of that field sits under `_chainLock` alongside the write, and the one in `GetState` deliberately does not, so it is the only read the lock does not order. Recorded here so they are not removed as noise for being hard to observe on the machine that runs the suite.
