...
_ownershipSync        object                   guards the owner exchange and the handler's record of it; the take and its own undo run under _sync, every other release does not, deliberately so for the context detach
...
- `_detached` is written and read under `_sync`, which is what pairs it with the append (see [Refusing a start for an attachment a detach already removed](#refusing-a-start-for-an-attachment-a-detach-already-removed)). `State` is the one read outside it, deliberately, so polling the handle never queues behind an append.
...
**What makes the property consistent is the shape of those writes, and no test pins that.** `State` and `Current` each take the instance and the phase in one load, so there is no read between those two for a transition to land in and no order to get wrong between them.

`State` is not one load in total. It takes the snapshot, and where that comes back settled it goes on to read `_detached` and then `Fault`, which is three loads on the path that reaches `Stopped`. Those two later loads are sound rather than exempt: they run only once the snapshot has come back holding no instance and raising no phase, and between them they can only choose among `Removed`, `Faulted` and `Stopped`. None of those three claims a transition is in flight, which is the claim a stale reading would have to make to be dangerous, so the worst either one does is report a settled target one moment older than it is.

The earlier design was the one that had a read order inside `State`. It held the instance and two flags in three fields, which forced a specific and asymmetric order on `State`: the start flag before the instance, the stop flag after it, each pairing an acquire with the release the body it belongs to had already made. That order was got wrong three times by three authors, in the version that introduced it, in the fix prescribed for that, and in a third gap measured afterwards where moving the stop flag's load above the instance left the entire suite green. Covering a window of that kind at all needs a seam inside the property, holding one of the reads open while a transition runs between it and the next. The pair is one load now, so there is nothing left for such a seam to sit between.

**The read order did not disappear, it moved to the consumer.** `HomeBlaze.OpcUa.SingleAttachmentHost.UpdateFromAttachment` polls the handle and carries two ordering rules of its own, neither of them pinned: the fault is read before any state is read, and each drop reads the state immediately before dropping rather than from one reading taken at the top of the method. Both are recorded in [what has no test behind it](#what-has-no-test-behind-it), with what goes wrong when either moves. The library's own read order is gone; a consumer that decides an irreversible act from a polled reading still has one, and always will, because nothing synchronizes a poll against the transition it races.
...
- the `Faulted` arm deleted from `State`, so a target carrying a fault settles to `Stopped`: `HostedServiceHandlerTests.WhenATransitionFaultedEarlier_ThenTheNextSuccessfulOneClearsTheFault`, which fails on the assertion rather than by running out of a wait;
...
- **The `Running` arm of `IsPublishing` in `HomeBlaze.OpcUa.SingleAttachmentHost`.** Dropping it, so that only `Starting` holds a drop back, leaves the suite green. It is load bearing all the same, and the comment on the not running branch gives the reason: that call site reads the state below a `Current` that came back null, so `Running` there means an instance arrived between the two reads and the tree under it is the one that instance has just published. Dropping that tree is the act this work exists to prevent, reached through a narrower interleaving than the original.
- **The exclusion of `Stopping` from `IsPublishing`.** Adding it, so that a stop in flight holds a drop back too, also leaves the suite green. The exclusion is a decision rather than an oversight and is recorded on the method: an instance that has left `Current` is being stopped and disposed, so what it published is the wrapper's to drop, where a start in flight may be publishing into the same tree the drop would clear. Including `Stopping` would defer every such drop to the first poll after the stop settles, which on the production interval is up to ten seconds of a stopped instance's tree still reading as the wrapper's own.
- **Where `UpdateFromAttachment` takes its readings**, which is two rules and no test for either. The fault is read first and the state after it, because a retry start clears the fault before it enters its start window, so a state read taken above the fault would still be settled for a fault this poll is about to act on. Each drop then takes its own state reading immediately before dropping, rather than the branches sharing one reading taken at the top: everything between the reading and the drop is window, and hoisting it there puts an intercepted property write in that window, which is long enough for a start to publish a whole tree into it. Both rules narrow the window rather than closing it, because a poll has no synchronization with the start it races, and nothing holds either in place: hoisting the state reading back above the fault read and sharing it between the two branches, which is both rules broken at once, leaves the whole suite green.
- **The `Volatile` qualifiers on `_snapshot`, `_fault` and `_detached`.** Dropping any of them to plain field access leaves the suite green, and a longer run on x64 would be a poor test of them: the loads and stores they pair are already ordered by that hardware, so what is left to go wrong is the JIT keeping a polled read in a register rather than taking it again, and the ordering on a weaker memory model. They are what makes a poll on an unrelated thread, which has no happens before edge to the transition body that wrote the field, a defined read rather than a race. `_detached` carries one because of where `State` reads it: every other read of that field sits under `_sync` alongside the write, and the one in `State` deliberately does not, so it is the only read the lock does not order. Recorded here so they are not removed as noise for being hard to observe on the machine that runs the suite.
