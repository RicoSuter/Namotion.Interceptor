...
- **Area:** `src/Namotion.Interceptor/` (the core library, .NET Standard 2.0). 36 files, 3,558 lines.
...
- **Written against:** `src/` at `b36ec531f`. This branch changes no file under `src/`, and `origin/master` at `569455abf` has the same `src/`, so the code described here is master's. Open pull requests that target a gap recorded here are cross-referenced where the gap is described; the description stays that of master.
- **Verified:** in three rounds, and no section is now unverified.
  ...
  - Round 3, 2026-09-23, one independent reviewer: a staleness pass against pull requests #594 to #597, opened after round 2, and a spot check of 36 load-bearing claims against the code, weighted toward candidate sizes and removal recommendations. 33 held. The three that did not are corrected: the Known broken gap count behind C1 read ten instead of nine, C18 called the zero-read-interceptor context "the common case" when no sample or host in the repository builds one, and one claim covering three whole-file sizes in section 1 (U9, U15, U16), which were `wc -l` derived and read one low. A review of that pass then corrected three cross-references it had not reached: PR #322 was called the fix for #410 though it acts only at the detach that reaches reference count 0, PR #594 was called the open fix for G17's second path though #489 also closes it, and section 8 said closing #412 left #402's defects with no open pull request in core though #595 now targets defect 2. A second review corrected two omissions those corrections left: #489 also acts on #410's partial detach, and #494 and #501 delete the fallback mutators #402's defects live in.
...
| | |
|---|---|
| Area | `src/Namotion.Interceptor/`, 36 files, 3,558 lines. One file, `InterceptorSubjectContext.cs`, is 1095 of them |
...
| Gaps | 22: 9 Known broken, 7 Unspecified, 5 Best effort, 1 Resolved. Ten come from the analysis in sections 2 to 5, twelve from open issues. [Section 6](#6-gaps-and-limitations) |
...
| Backlog | 34 items dispositioned: 14 issues and 20 of the 63 pull requests open at the 2026-09-21 sweep. Four pull requests opened since, #594 to #597, are cross-referenced at the gaps they target and not counted. [Section 8](#8-backlog-disposition) |
...
2. **Whether a read is synchronized is decided by the registered service set, not by the contract.** A read takes `SyncRoot` only when at least one read interceptor is registered. Contradiction [X1](#contradictions), with all nine cells in the [lock coverage matrix](#lock-coverage). PR #596 is open and implements the defect reading of it, see gap [G1](#6-gaps-and-limitations).
3. **A detach lifecycle callback that removes the same fallback again does not terminate.** The membership test it passes is unlocked, and the registration it would clear is only cleared after the callbacks have run. [Reentrancy](#reentrancy). PR #595 reproduces it as a stack overflow that takes the process down and is an open fix, see gap [G9](#6-gaps-and-limitations).
...
- **Both defects this document found in itself are now fixed**, and recorded rather than erased: [G10](#6-gaps-and-limitations), where section 3's totals sentence mis-summed its own evidence, and the `wc -l` derived chain-class total that read two low. See [section 7](#7-candidates)'s counting convention for why fourteen of core's 36 files make `wc -l` read one short.
...
| I63 | On an executor, lifecycle interceptors are attached after the fallback context is added and detached before it is removed, so a callback always runs while the topology still describes the relationship. | `src/Namotion.Interceptor/Interceptors/InterceptorExecutor.cs:113`, `:120`, `:135`, `:138` | Unverified. PR #595 adds a test that pins it |
...
Reading the table: Implementations is the count the subsection's own assessment states, so a row above 1 is one of the 13 the totals at the end of this section name. Split justified means the code states a reason for the duplication. Candidate means the assessment calls it something that should be one, which is a reading of the evidence and not a ruling: [section 7](#7-candidates) ranks the candidates, and only the maintainer rules on them.
...
**Totals.** 24 concepts. 13 have more than one implementation: context, service ordering, delegation target, interceptor chain, terminal operation, chain cache, subject data, property metadata, change origin, commit revision, timestamp, ambient scope, thread-static channel. Of those, **two** are justified in writing and are not findings: property metadata (`SubjectPropertyMetadata.cs:97`) and commit revision (`PropertyWriteState.cs:45`). **Two more are justified only in part**, so they remain findings: delegation target, where the memoized chain end is justified (`InterceptorSubjectContext.cs:970`) and the four-times-repeated resolution prologue is not, and thread-static channel, where the code explains two of the three divergences (`:869` and `:580`) and not the third. That leaves **11 concepts this section reads as findings**, which is what the Candidate column above counts. Timestamp is not among the justified: `SubjectChangeContext.cs:72` steers usage between the two implementations rather than justifying that there are two.
...
An arrow means the lock at the tail may be held while the lock at the head is acquired. The dotted path is a reentrant recursion, which the rows under the diagram carry the citations for.
...
| A detach lifecycle callback calls `RemoveFallbackContext` again for the same pair | the lifecycle callback (`src/Namotion.Interceptor/Interceptors/InterceptorExecutor.cs:135`) | none. The membership test it passes is unlocked (`src/Namotion.Interceptor/Interceptors/InterceptorExecutor.cs:129`, reading `InterceptorSubjectContext.cs:152`) | **No, and it does not terminate.** PR #595 reproduces the stack overflow and is the open fix ([G9](#6-gaps-and-limitations)) |
| Two threads remove the same fallback concurrently | the detach callbacks, on both threads | none. The same unlocked membership test | **No.** Nothing in the contract says a detach callback fires at most once (`src/Namotion.Interceptor/Interceptors/ILifecycleInterceptor.cs:11` to `:15`). PR #595 makes it at most once per pair |
...
| # | Gap | Kind | Issue |
|---|---|---|---|
| G1 | Whether a read is synchronized against a concurrent write is decided by the registered service set. `IInterceptorSubject.SyncRoot` documents the lock unconditionally (`src/Namotion.Interceptor/IInterceptorSubject.cs:8`), the zero-interceptor read terminal takes nothing (`src/Namotion.Interceptor/Cache/ReadInterceptorFactory.cs:12`). Contradiction [X1](#contradictions), no verdict. PR #596 is open and implements the defect reading: it reproduces torn reads of a 128-byte struct through this terminal and makes both read terminals lock. PR #597, stacked on it, then skips the lock on both for types the runtime reads atomically. See [C18](#c18-x1-the-read-terminal-pair). | Unspecified | none |
...
| G9 | The detach path does not terminate, it does not merely fire twice. A detach lifecycle callback that calls `RemoveFallbackContext` again for the same pair passes the unlocked membership test (`src/Namotion.Interceptor/Interceptors/InterceptorExecutor.cs:129`, reading `src/Namotion.Interceptor/InterceptorSubjectContext.cs:152`), and the base mutator that would clear the registration is only reached afterwards (`src/Namotion.Interceptor/Interceptors/InterceptorExecutor.cs:138`), so it recurses until the stack is gone. [Reentrancy](#reentrancy). #402 records the duplicate fire as its defect 2 and does not record the non-termination, so the issue understates this. PR #595 is an open fix for both: it reproduces the recursion as a stack overflow that takes the process down, and claims the pair before running the callbacks so a second removal of the same pair returns false. It leaves G11 and G12 open. | Known broken | #402 |
| G10 | [Section 3](#3-concept-census)'s totals sentence mis-summed its own evidence: it named delegation target among the concepts "justified in writing and are not findings", while that subsection's own assessment calls the four-times-repeated resolution prologue a finding and the at-a-glance table marks the row Candidate: Yes. Counting the Candidate column gave 11 while the sentence implied 10. **Resolved 2026-09-21**: the sentence now states two fully justified concepts, two justified only in part, and 11 findings, which reconciles it with the table. Recorded rather than deleted because it is evidence about how this document was produced. | Resolved | none |
...
| G17 | A detach that leaves property values set never calls `RemoveFallbackContext`, so the edge is stranded: a detached subtree keeps resolving services through its former parent, and a mutually referencing pair can become a pure delegation cycle nobody constructed, which then raises on every read, write, invoke and service query (I11, I12). The deciding code is `ContextInheritanceHandler` in Tracking and is outside this dossier's boundary; the state it leaves behind is inside it. The same handler strands the edge a second way, on a detach that does reach reference count 0: it composes the context of the parent the subject first attached through (`src/Namotion.Interceptor.Tracking/Lifecycle/ContextInheritanceHandler.cs:19`) and removes the context of the parent whose removal was the last detach (`:23`). For a subject shared by two different parent subjects those differ whenever the parent it attached through lets go first: the removal matches nothing and returns false (`src/Namotion.Interceptor/Interceptors/InterceptorExecutor.cs:129`, `:141`), and the detached subject keeps resolving the graph's services. PR #594 is an open fix for that second path and leaves #410's path untouched. PR #489, a draft feature pull request that does not target `master`, also closes it by rewriting the handler to decompose the context the subject inherited from, and it acts on #410's path too: when the parent a subject inherits from lets go while another parent still holds the subject, the handler moves the inherited context to that parent, or to the chain's root when none is left. #494 and #501 delete the handler with C1. | Known broken | #410 |
...
22 gaps: 9 Known broken, 7 Unspecified, 5 Best effort, 1 Resolved (G10, a defect in this document, fixed on 2026-09-21 and kept as a row so the correction is visible).
...
Counting convention, because it moves several figures by one each. A whole-file size here is the number of lines of text in the file. Fourteen of core's 36 files end without a trailing newline, so `wc -l` reports one fewer than the file has: the area header's 3,558 is the real total, where `wc -l` reports 3,544. Section 3's figure for the three chain classes was `wc -l` derived and read two low; it is now stated as 223, the real count.
...
- **Recommendation:** conditional, and the condition is his to set.
  ...
  Two open pull requests bear on this without ruling it. PR #596 implements the defect row: both read terminals go through one locking method. It reproduces torn reads of a 128-byte struct through the zero-interceptor terminal and prices the lock at +7.8% on `SubjectUpdateBenchmark.CreateCompleteUpdate`, the one benchmark that reads properties through a context with no read interceptor (`src/Namotion.Interceptor.Benchmark/SubjectUpdateBenchmark.cs:22` to `:24`; `PropertyChangeSubscriptionsBenchmark.cs:255` to `:257` builds one too, but only writes). PR #597, stacked on it, is an answer the table does not list: it skips `SyncRoot` on both read terminals for types the runtime reads atomically, references and primitives or enums no wider than a pointer. Reads then still never tear, and give up the acquire fence the lock carried. Because it changes the chained terminal too, it reaches every context built with `WithFullPropertyTracking()`, not only the zero-interceptor one.
...
| #410 | Gap G17 | The deciding code, `ContextInheritanceHandler`, is in Tracking and outside this dossier's boundary; the stranded edge it leaves behind is inside it. PR #322 does not reach it: it removes contexts only at the detach that reaches reference count 0, which this path never does. PR #594 fixes the sibling path G17 also records, a last detach through a different parent, and not this one. PR #489, a draft that does not target `master`, does reach it: its handler moves the inherited context off a parent that lets go while the subject stays referenced. #494 and #501 delete the handler with C1. |
...
| #322 | 2 | Gap G17 | Not a fix for #410: it snapshots the parent's ancestor chain at first property attach and removes those ancestors on last detach, the detach that reaches reference count 0, which #410's partial detach never does. Nor does it close G17's second path, because the direct parent's context is still removed through whichever parent let go last. What it removes is the leaked constructor context #207 records, the same stranded edge reached a third way: the handler removes the parent's context from a subject whose edge is to a context the handler never composed. It adds `GetFallbackContexts()` to the public `IInterceptorSubjectContext` (+6), a member C1 deletes. |
...
**One caveat before closing it.** #419 currently contains three files, two specifications and one plan under `docs/superpowers/`, and zero files under `src/`. What supersedes #412 today is therefore a design and a plan, not an implementation. #419 is also stacked on #474, which this table puts on C1. Closing #412 now leaves no open pull request that fixes #402's defects in place, except defect 2, which #595, opened after the sweep, targets. #494 and #501 instead delete the fallback mutators they live in, as part of C1.

#### Pull requests opened after the sweep

Four pull requests opened on 2026-09-22, each against a gap this document records: three titled as fixes, and #597, a performance change stacked on #596. They are cross-referenced where the gap is described and are not dispositioned or counted here. #596 and #597 bear on [C18](#c18-x1-the-read-terminal-pair)'s ruling without making it, and C18 records how.

| PR | Core files | Targets | What it does to the recorded gap |
|---|---|---|---|
| #594 | 0 | [G17](#6-gaps-and-limitations), the sibling path | Records the context `ContextInheritanceHandler` composed and decomposes exactly that one at the last detach. Tracking only. Leaves #410's stranded edge and the hand-composed case (#207) as they are. |
| #595 | 2 | [G9](#6-gaps-and-limitations), I63 | Claims the executor and fallback pair before the detach callbacks run, so a re-entered or concurrent removal of the same pair returns false and the callbacks fire at most once. Leaves G11 and G12 open. |
| #596 | 1 | [G1](#6-gaps-and-limitations), X1, [C18](#c18-x1-the-read-terminal-pair) | Makes the zero-interceptor read terminal lock, which is C18's defect row. |
| #597 | 3 | [G1](#6-gaps-and-limitations), X1, [C18](#c18-x1-the-read-terminal-pair) | Stacked on #596. Skips the read lock on both terminals for types the runtime reads atomically, which C18's table does not list. |
