...
Verified: 2026-09-20, four independent readers over sections 2, 3, 4 and 5. 181 claims checked, 154 confirmed in substance, 63 corrected in substance, wording or citation, 13 claims added that no reader had found. The two counts overlap: 27 claims failed on substance, the rest of the 63 were confirmed in substance and corrected in wording or citation.
...
What core promises, in the caller's words. Every public member of `src/Namotion.Interceptor.Tests/VerifyChecksTests.PublicApi.verified.txt` maps to at least one row below; the coverage notes after the table name the rows whose capability has no demonstrated caller. Line counts are whole-file sizes of `src/Namotion.Interceptor/` unless a sub-range from section 3 or 5 is cited.
...
| U9 | A context is configured fluently, and a `WithX()` extension registers its service exactly once whatever order the extensions are called in. | Undecided | `TryAddService` (`InterceptorSubjectContext.cs:187`), `AddService` (`:213`), the retrieval pair (`:107`, `:224`) and three one-line wrappers (`InterceptorSubjectContextExtensions.cs:16`, `:31`, `:45`, 49 lines). `TryAddService` holds `_mutationLock` across both caller-supplied delegates (`:195`, `:200`), which is gaps [G13](#6-gaps-and-limitations), G14 and G16. | |
...
| U12 | A caller can decide the timestamp a write is stamped with, or replace the clock process wide. | Undecided | `SubjectChangeContext.cs`, 149 lines: the ambient scope pair (`:115`, `:130`), the single capture point (`:45`) and the process-wide settable clock (`:33`). The same three-way sentinel decision is implemented a second time on the write context (`Interceptors/IWriteInterceptor.cs:207`), which section 3 calls a finding. Section 5 records that the caller's function runs under the subject's `SyncRoot` and that nothing documents it. | |
| U13 | A source marks a property it has already published, so a later change can be filtered out of the echo back to it. | Undecided | One one-way volatile flag (`PropertyWriteState.cs:58`), set at `PropertyReference.cs:168` and returned from `TryGetWriteState` (`:134`). Deliberately not per source, which is what lets it hold no source reference to release on detach (I42, Unverified). | |
...
| U17 | A property's attributes include the ones declared on the interfaces it implements, deduplicated by `AllowMultiple`. | Undecided | `PropertyInfoExtensions.cs`, 102 lines, with two process-wide caches (`:14`, `:92`). The dedup half of I62 has a test; the ordering half does not, per gap [G7](#6-gaps-and-limitations). | |
...
| U22 | Read, write and invoke are driven by property name from outside the generated members. | Undecided | `Interceptors/IInterceptorExecutor.cs` (32 lines) plus the three entry points and the compare-and-swap publisher on `Interceptors/InterceptorExecutor.cs` (142 lines). The interface carries a standing removal note in the code (`Interceptors/IInterceptorExecutor.cs:3`) and has two live consumers outside core. Section 3, [context](#context). | |
...
| U24 | A service query never blocks and never deadlocks against a concurrent topology mutation, including in a cyclic fallback graph. | Undecided | The immutable state snapshot published by `Interlocked.Exchange` under `_mutationLock` (`InterceptorSubjectContext.cs:797`, `:799`), the upward invalidation walk (`:779-943`, 165 lines), the per-context used-by set (`:80`) and five thread-static traversal buffers whose retain-or-drop policy is written out three times (`:324`, `:590`, `:871`). Part of the same 666 lines U2 pays. I1, I2. | |
...
- Every public type and member in the API snapshot maps to at least one row. Nothing was left out.
...
- **One capability's entry point is not public at all.** U14 can be named from outside but not applied: `ChangeOrigin.FromSource` (`ChangeOrigin.cs:50`) and `ChangeOrigin.Confirmed` (`:57`) are public, while the slot that carries a stamp into a write is `internal` (`PendingOrigin.cs:21`, `:37`), reachable only through `InternalsVisibleTo` for `Namotion.Interceptor.Tracking` (`src/Namotion.Interceptor/Namotion.Interceptor.csproj:18`). A library outside that list can construct an origin and cannot stamp one.
...
Two sources. G1 to G10 are what the analysis in sections 2 to 5 found, including three defects in the analysis itself. G11 to G22 come from the fourteen open issues that describe core. Kind is this document's classification, not a maintainer ruling.
...
| G6 | The headline "42 covered" reads stronger than the rows. Nine of the 42 carry a parenthetical saying the named test pins only part of the claim: I5, I16, I25, I26, I43, I46, I50, I53 and I64. The untested halves include "exactly one attempt and no retry" (I5), "returns `default` for none" (I16), "the timestamp is recorded either way" (I25), the `Confirmed` inclusion (I26) and whether `FinalizeOrigin` and `SetWriteState` sit inside the lock (I50). | Best effort | none |
| G7 | Two tests cannot fail on what their names claim, verified against the source for this section. `MultipleInterfaces_FirstInterfaceWins_WhenAllowMultipleFalse` (`src/Namotion.Interceptor.Tests/PropertyInfoExtensionsTests.cs:238`) asserts `single.Value == "first" \|\| single.Value == "second"` (`:248`), which holds whichever interface wins. `CombinedInheritance_ClassAndInterface_CorrectOrder` (`:336`) asserts `Assert.Single(defaults)` then the value of `defaults[0]` (`:347`), a value on a one-element list rather than a position. The ordering half of I62 is therefore unpinned, and the rule it would pin (`src/Namotion.Interceptor/PropertyInfoExtensions.cs:25` to `:26`) is exactly what #539 wants to redefine. | Known broken | #539 |
...
| G18 | Two memory characteristics of the copy-on-write state are deliberate and unmeasured. The compiled chain arrays are sized by a process-wide counter that never resets (`src/Namotion.Interceptor/InterceptorSubjectContext.cs:48`, grown at `:1083`), so a terminal context's array is as long as the largest property type index the process ever issued. And `WithoutCaches` must always allocate (I7, `:1002`), so invalidating a delegating context now costs one state object where the previous design cost nothing. Neither affects correctness; neither has a number. | Best effort | #409 |
