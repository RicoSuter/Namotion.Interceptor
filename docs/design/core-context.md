...
3. **A detach lifecycle callback that removes the same fallback again does not terminate.** The membership test it passes is unlocked, and the registration it would clear is only cleared after the callbacks have run. [Reentrancy](#reentrancy). PR #595 reproduces it as a stack overflow that takes the process down and is the open fix, see gap [G9](#6-gaps-and-limitations).
