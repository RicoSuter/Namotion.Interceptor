...
- **Concurrent `RecalculateDerivedProperty`**: Another write triggered recalculation but the current one is in progress, so the concurrent call bails and signals the flag. It also hands off its timestamps, so the value the owner commits next carries the timestamp of the last write it reflects. The last hand-off wins rather than the latest timestamp, because timestamps are not monotonic (source timestamps, `WithChangedTimestamp` scopes) and a serial execution would also stamp the last write's.
