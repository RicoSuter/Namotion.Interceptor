...
- **`WithLocalOrigin()`**: Wraps the notification in a local-origin scope (`Source = null`, ambient timestamps preserved). The local model computed the value and no source confirmed it, so the change flows to bound sources like any local write (derived recalculations are never echo-suppressed).
