...
The per-accessor pair (`GetAccessorAccessibility` in `SymbolExtensions.cs`) exists because a getter and setter can have independently weaker accessibility than the property itself; the emitter drops just the inaccessible half rather than the whole member.
...
**One helper, two call sites.** `InterfacePropertyMetadataExtractor` (interface default properties) and `ClassPropertyMetadataExtractor` (class-declared explicit implementations, `string IFoo.Kind => ...` written directly on the subject class) both call the shared `GetAccessorAccessibility` rather than carrying a copy each. Two copies drift, and that drift is how the same CS1540 for a class explicitly implementing a protected interface member survived on the class-declared path while the interface-default path was being fixed: the fix did not touch the code path that also needed it.

The lesson generalises beyond this one guard: an accessibility rule that does not go through `IsSymbolAccessibleWithin`, or an equivalent compiler query, is testing a proxy for the real question. The individual steps (the exemption, the implemented member against the list, the symbol query that ships and the shared helper) landed squashed in #432.
