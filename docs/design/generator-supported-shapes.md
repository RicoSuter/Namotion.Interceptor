...
**Root mode** emits everything: `_context`, `_properties`, the explicit `Executor`, `Data`, `Properties` and `AddProperties`, plus `GetInstanceProperties()` and the three helpers `GetPropertyValue`, `SetPropertyValue` and `InvokeMethod`. The helpers changed from `private` to `protected` so a subject below can call them. `_context` and `_properties` stay private, because only the root's own members touch them.
