...
### SourceTracker as an explicitly configured context service

The tracker lives in the `IInterceptorSubjectContext`, not in DI, and is added explicitly at context configuration time:

```csharp
public static IInterceptorSubjectContext WithSourceTracking(this IInterceptorSubjectContext context)
```

called on the tree root context alongside the other features (same shape as the existing `WithSourceTransactions()`). Automatic placement from inside a source cannot work: connectors receive the context of their own root subject, which can be a subtree context, and a service added there is invisible to the tree root and to sibling subtrees (context fallbacks point child to parent, never sideways). A subtree-rooted source would fragment the tree into per-subtree trackers, and a waiter on the root context would create yet another empty tracker and block forever. Explicit configuration puts the tracker at the scope the whole tree resolves.

`SubjectSourceBase` overrides `StartAsync`: it resolves all trackers reachable from its context (`GetServices<SourceTracker>()` walks the fallback chain upward, so a subtree-rooted source finds the root tracker), registers with each, and only then calls `base.StartAsync` to launch the pump. If no tracker is resolvable, the source runs untracked; existing applications and benchmarks that never call `WithSourceTracking()` keep working unchanged. On `Dispose` the source unregisters from the trackers it registered with. Because both DI-hosted and dynamically created sources start through the same hosted-service lifecycle, registration works for both.
...
Custom `ISubjectSource` implementations that do not derive from `SubjectSourceBase` register themselves with the resolvable trackers once fully constructed and started, and unregister on dispose.
...
### Tracker access and the startup race

The wait methods live only on the tracker; consumers resolve the tracker and are responsible for picking the right one. A single accessor extension in `Namotion.Interceptor.Connectors` exists purely for discoverable error messages:

```csharp
public static SourceTracker GetSourceTracker(this IInterceptorSubjectContext context)
```

It resolves through the fallback chain (so calling it on a child subject's context finds the tree tracker). If no tracker is resolvable, it throws `InvalidOperationException` with guidance to call `WithSourceTracking()` on the tree root context; a missing tracker would otherwise surface as a silent forever-block. If multiple trackers are resolvable (one configured locally and one on a shared ancestor context, or a multi-parent subject attached to two trees), it throws and directs the caller to `GetServices<SourceTracker>()`: combining trackers is a semantic decision that belongs at the callsite, for example `Task.WhenAll` over each tracker's wait to mean "all trees are live". Each tracker applies the empty-set rule to its own sources, so waiting on a configured tree before any source exists blocks until the first source registers.
...
await context.GetSourceTracker().WaitForAllSourcesSynchronizedAsync(cancellationToken);
...
The concrete consumer this design must serve well, and the scenario that motivated the queued delivery model: an application declares a stored `ConnectionState` registry attribute (of type `SourceState`) on selected properties and a derived `IsAvailable` attribute computing `ConnectionState == SourceState.Synchronized`. A small updater component subscribes to the tracker stream once and maintains `ConnectionState`; the existing derived-property infrastructure propagates `IsAvailable` changes to UIs and GraphQL automatically. The registry already supports both halves (`AddAttribute` or `[PropertyAttribute]` for the stored attribute, `AddDerivedAttribute` or `[Derived]` for the derived one).
...
One pull request implements the whole design, based on master (the design touches only the core and Connectors packages plus docs; the in-flight OPC UA loader work touches neither). The PR updates `docs/connectors.md` as the permanent documentation, including the availability-attributes scenario above as a worked sample (stored `ConnectionState` attribute, derived `IsAvailable`, updater subscribing to the tracker stream and writing directly in the callback), and deletes this spec at the end, once the design is implemented.
...
- `Namotion.Interceptor.Connectors`: `SourceState`, `SourceEventKind`, `SourceEvent`, `ISubjectSource` members (`State`, `LastSynchronizedAt`, `PendingWriteCount`, `StateChanged`), `SubjectSourceBase` transitions (pump entry, catch, finally, disposal fallback) and `StartAsync` registration, `SourceTracker` (including the lifecycle attach/detach catch-up and internal wait bookkeeping), the `WithSourceTracking()` context extension, the `GetSourceTracker()` accessor extension, `GetSourceState` property extension, emission logic in `SetSource`/`RemoveSource` (built on `TryAddPropertyData`).
...
- `docs/connectors.md`: new "Source state and events" section covering the state model, the stream, the consumer protocol (including the hosted-waiter `ApplicationStarted` guidance), and the breaking-change note for custom `ISubjectSource` implementers; the context recipe gains `WithSourceTracking()`.
...
- Tracker robustness: a tracker on a context without lifecycle tracking supports registration, stream, and waits without throwing (catch-up disabled); a claim on a subject attached to two trees with different trackers throws nothing and appears on both streams; a source started on a context without any tracker runs untracked and does not throw; `GetSourceTracker()` on a context without a tracker throws `InvalidOperationException` with configuration guidance; with trackers on both a local and an ancestor context it throws, while `GetServices<SourceTracker>()` returns both so the consumer can combine waits explicitly.
...
- Auto-registered tracker (source adds the tracker to its own context via `TryAddService` on start, no explicit configuration): `TryAddService` cannot place a service at tree scope from below. A subtree-rooted source would add the tracker to its subtree context, where the tree root and sibling subtrees cannot resolve it (fallbacks point child to parent); trackers fragment per subtree, and a waiter on the root context creates yet another empty tracker and blocks forever. Rejected in favor of explicit `WithSourceTracking()` configuration on the tree root context, matching the `WithLifecycle()` requirement sources already have.
- Serialized (totally ordered) stream emission through a tracker-wide lock: preventing same-property claim/release inversion would require the emission lock to span the ownership compare-and-set, nesting a new lock inside the lifecycle lock and the public claim primitives. Deferred; the documented weaker contract (notification stream plus authoritative reads) suffices, and strengthening delivery guarantees later is non-breaking.
...
- Tracker as a DI singleton seeded from `IEnumerable<IHostedService>`: works for DI sources but requires a new DI registration and does not naturally cover dynamically created sources or multiple trees. Rejected in favor of the explicitly configured context service.
