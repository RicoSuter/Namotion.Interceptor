...
The `IServiceCollection` overload additionally registers a hosted service whose only job is to call `CompleteSourceRegistration()` when `IHostApplicationLifetime.ApplicationStarted` fires. It follows the shape of the existing `WithHostedServices(IServiceCollection)`. It is order-independent for a simpler reason than host construction order: the monitor takes its initial registration count at `WithSourceMonitoring()` time, during context configuration, before the host is even built. No source can therefore complete a wait before the signal, whatever order services were registered in. The holder takes no hold of its own; it only releases the initial one.
