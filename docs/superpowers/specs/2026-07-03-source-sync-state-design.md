...
public enum SourceState
{
    ...
    Stopped        // source shut down (host stop or dynamic removal); final, a stopped instance is never restarted
}
```
...
- `StateChanged` is the notification primitive: the source raises it itself whenever `State` changes, with the same `SourceEvent` payload used on the stream (no separate event args type). Raising the event is part of the interface contract and is the idiomatic thing implementers get right; sources never report state to the monitor directly. Unlike the monitor stream, this event is raised synchronously on the transitioning thread (normally the pump) and inside the source's transition lock. Handlers **must** be observe-only: they must not block, and must not cause a transition of any source, directly or indirectly. Mutating consumers belong on the monitor stream, where delivery is queued and outside all locks.
...
- The state is stored in an `int` field (the enum's underlying value) so it works with `Volatile.Read` and `Interlocked.CompareExchange`. All transitions go through a helper with two rules: a transition to the current state is a no-op (no duplicate events), and **`Stopped` is terminal**. Nothing leaves it.
  ...
  Terminal by **enforcement**, not by assumption. `SubjectSourceBase.StartAsync` refuses to start a source whose state is already `Stopped`: it logs and returns without launching the pump, so a stopped instance can never re-enter `Connecting`.

  The guard is required, because the platform does not provide this. `BackgroundService.StartAsync` creates a **fresh** linked `CancellationTokenSource` on every call and `StopAsync` cancels only the previous one, so a second `StartAsync` on the same instance runs against an uncancelled token and `ExecuteAsync` runs again. Verified empirically against Microsoft.Extensions.Hosting.Abstractions 9.0.10, the version this repository resolves: start, stop, start on one instance yields two `ExecuteAsync` runs, the second entered with a token that is not cancelled. An earlier revision of this section claimed the opposite and was wrong.

  What is true is that the *automatic* path never restarts: `HostedServiceHandler` removes the service from the subject's attached list on detach, so a re-attached subject does not try to start it again, and `HostedServiceHandlerTests.cs:108` asserts exactly that with the comment "the service has been stopped and removed from list (not allowed to restart again anyway)". That covers re-attach but not an application that retained the instance and calls `AttachHostedService(sameInstance)` explicitly, which re-adds it to the cleared list and posts a start. Without the guard that source would claim, load and apply live values while `State` stayed pinned at `Stopped`, so waits would vacuously ignore a live source and `GetSourceState()` would report `Stopped` on actively synchronizing properties: the inverse of the outage defect this design exists to fix.

  Restart is therefore not supported, matching the position `HostedServiceHandler` already takes, and the guard makes that position true rather than hoped for. An application needing a fresh connection creates a new source instance.
- **The transition, the timestamp write, and the event publication are serialized against each other under a per-source lock.** Compare-exchange alone is not sufficient, because the three steps are not atomic: the writer can compare-exchange `Connecting` to `Synchronized`, be preempted, let disposal compare-exchange `Synchronized` to `Stopped` and unregister, then resume to write `LastSynchronizedAt` and raise `Synchronized` after `Stopped` has already been published. Both compare-exchanges succeed, so stickiness cannot prevent it, and subscribers observe reversed transitions while the monitor may have already unsubscribed and missed one entirely. Holding a per-source lock across read-decide-write-stamp-publish removes the window. Contention is negligible: transitions happen on connect cycles, not on the data path, and monitor delivery is queued, so stream subscribers never run under the lock.

  Direct `ISubjectSource.StateChanged` subscribers **do** run under it, because that event is raised synchronously on the transitioning thread and the raise is inside the serialized region. This is why the `StateChanged` contract below is a must rather than a should: a blocking handler stalls the pump and disposal, and a handler that triggers a transition of the same source reenters the lock (.NET monitors are reentrant) and publishes a nested, out-of-order event, reopening the exact window the lock closes. Moving the raise outside the lock is worse, since it restores reversed publication for those same subscribers.
    ...
    DateTimeOffset Timestamp)
{
    ...
    /// </remarks>
    public SourceState CurrentState => ResolveCurrentState();   // computed, never an auto-property
}
```
...
The `IServiceCollection` overload additionally registers a hosted service whose only job is to complete source registration once startup has settled. Signalling on `IHostApplicationLifetime.ApplicationStarted` alone is **not** sufficient, and getting this wrong would reintroduce the exact defect the registration count replaced. `ApplicationStarted` fires when `Host.StartAsync` completes, but `HostedServiceHandler.StartAsync` only launches its drain loop and returns, while the start actions it has queued each await `Task.Delay(50)` and run sequentially afterwards. An application that builds a tree and attaches sources through the lifecycle path therefore has those sources registering tens or hundreds of milliseconds *after* `ApplicationStarted`. If one DI-registered source has synchronized by then, all three wait conditions hold and the wait completes on a partially registered set.

So the holder awaits the attach queue before signalling. It cannot reference `HostedServiceHandler` directly, because `Namotion.Interceptor.Connectors` does not reference `Namotion.Interceptor.Hosting` and inverting that layering was rejected. Instead `Namotion.Interceptor.Tracking`, which both packages already reference, defines the barrier:

```csharp
namespace Namotion.Interceptor.Tracking.Lifecycle;

/// <summary>Queued lifecycle-driven work that a caller can wait to have completed.</summary>
public interface IPendingActionsBarrier
{
    Task WaitForPendingActionsAsync(CancellationToken cancellationToken);
}
```

`HostedServiceHandler` implements it, and since it is already registered as a context service, `GetServices<IPendingActionsBarrier>()` resolves it by assignability with no new registration. On `ApplicationStarted` the holder awaits every reachable barrier and only then calls `CompleteSourceRegistration()`. A tree with no such barrier resolves none and signals immediately, so the DI-only case is unchanged. It follows the shape of the existing `WithHostedServices(IServiceCollection)`. It is order-independent for a simpler reason than host construction order: the monitor takes its initial registration count at `WithSourceMonitoring()` time, during context configuration, before the host is even built. No source can therefore complete a wait before the signal, whatever order services were registered in. The holder takes no hold of its own; it only releases the initial one.

`SubjectSourceBase` overrides `StartAsync`. It first applies the terminal-`Stopped` guard and returns immediately if the source has already stopped, **before** touching the monitors: a guard placed after registration would re-register a permanently `Stopped` source, which would then sit in `Sources` forever and make a wait on its branch complete vacuously instead of blocking on an empty scope. Only then does it resolve all monitors reachable from its context (`GetServices<SourceMonitor>()` walks the fallback chain upward, so a subtree-rooted source finds the root monitor), register with each, and call `base.StartAsync` to launch the pump. If no monitor is resolvable, the source runs untracked; existing applications and benchmarks that never call `WithSourceMonitoring()` keep working unchanged. On `Dispose` the source unregisters from the monitors it registered with. Because both DI-hosted and dynamically created sources start through the same hosted-service lifecycle, registration works for both.
...
Two robustness contracts: the monitor never throws for a tree whose subjects simply never attach through the lifecycle path, in which case the catch-up scan finds nothing while registration, stream, and waits work normally; and `Unregister` of a source that was never registered is a no-op.

An earlier revision promised the monitor worked on a context with no lifecycle interceptor at all. That configuration cannot exist: `WithSourceMonitoring` implies `WithParents()`, and `WithParents()` returns `context.WithLifecycle()` (`src/Namotion.Interceptor.Tracking/InterceptorSubjectContextExtensions.cs:171-178`), so every monitor-bearing context has one.
...
Both tests are the same helper with its arguments swapped: `IsAncestorOrSelf(candidate, target)` walks up from `target` through `subject.GetParents()` (from `WithParents()` in Tracking) with a visited set, since the parent graph is a multi-parent DAG. Ancestor chains are shallow and the walk runs only on wait start and on wait notifications, which are rare. An anchor with no parents matches no source rooted *elsewhere*, since the upward walk finds nothing. It can still self-match: a source whose `RootSubject` is the anchor itself satisfies the ancestor-or-self test without any walk. That case is real and is covered by the vacuous-completion rule below, not by blocking.
...
- The monitor resolves its logger lazily through a `Func<ILogger?>`, the same idiom `HostedServiceHandler` uses, because the context is configured before any logging provider exists. Without it every warning below is a silent no-op, and warnings are the only thing standing between a vacuous completion and a consumer that thinks its tree is live.
- Implementation is a re-evaluation over a `TaskCompletionSource` (created with `RunContinuationsAsynchronously`), triggered by registration, unregistration, registration-count changes, state changes, **and any lifecycle change that mutates the parent graph**.
- The topology trigger is not optional, and it must be wider than context attach and detach. Branch scope is computed from `GetParents()`, which is mutable: reparenting a wait's anchor, or a source's `RootSubject`, changes which sources are in scope without any registration or state transition occurring. A reparent **within** the same tree fires neither: `IsContextAttach` fires only on reference count 0 to 1 and `IsContextDetach` only on reaching 0, while `ParentTrackingHandler` updates parents on `IsPropertyReferenceAdded` and `IsPropertyReferenceRemoved` (`src/Namotion.Interceptor.Tracking/Parent/ParentTrackingHandler.cs:19-29`). Hooking only context attach and detach would therefore miss the precise scenario that motivates the trigger, so the monitor hooks parent-reference mutation. Without a topology trigger a pending wait's condition can become satisfied, or become unsatisfiable, while its `TaskCompletionSource` is never re-evaluated, leaving it blocked indefinitely. The monitor already hooks lifecycle for the catch-up scan, so this reuses an existing subscription rather than adding one. Unlike the catch-up scan, this trigger is **not** gated on having stream subscribers, because a wait is exactly the consumer that has none.
...
- A detached anchor whose own source is still registered completes vacuously rather than blocking. Detaching a subject stops its attached source, but unregistration happens on dispose, so the stopped source stays registered with `RootSubject` equal to the anchor. It self-matches, every in-scope source is `Stopped`, and condition 3 is vacuously met. The warning below is what makes this visible.
...
- **On subject detach**: emit `PropertyLeftView` for any still claimed. The scan does not remove the ownership data, because the source has not released anything and `ChangeQueueProcessor` filters outbound changes on `TryGetSource(...) == this`; clearing it would silently stop a re-attached subject from reaching its source. `CurrentState` resolves to `Unclaimed` because the monitor is no longer resolvable from the detached subject's context, so a view maintainer converges even though a plain ownership read would still report the source. Releases performed by `SourceOwnershipManager` during detach go through `RemoveSource` and are real `PropertyReleased` events; the scan then finds nothing, so there are no duplicates. That no-duplicate property depends on ordering and must not be left to accident: the monitor hooks `ILifecycleHandler.HandleLifecycleChange`, not `LifecycleInterceptor.SubjectDetaching`, because the interceptor raises `SubjectDetaching` (which the ownership manager uses, `SourceOwnershipManager.cs:52`) before invoking the lifecycle handlers, so the manager's releases are guaranteed to precede the scan.

**Ordering against `ContextInheritanceHandler` is load-bearing and must be declared.** The topology-aware `CurrentState` reads whether the monitor is still resolvable from the subject's context, and that resolvability is mutated by another `ILifecycleHandler` in the same invocation pass: `ContextInheritanceHandler` adds the parent fallback on attach and removes it on the last detach. Handlers run in array order, which for services no ordering attribute separates is registration order, so without a declared constraint the answer depends on whether the application called `WithSourceMonitoring()` before or after `WithFullPropertyTracking()`. If the scan runs first, the event is enqueued before the fallback is added or removed, and because delivery is on another thread the emitting thread can be preempted inside that window: the handler then reads `Unclaimed` for `PropertyEnteredView`, or the owning source for `PropertyLeftView`. Either is the last event for that property, so the view diverges permanently, which is the quiescent-consistency failure this design exists to prevent.

The monitor's lifecycle hook therefore carries `[RunsAfter(typeof(ContextInheritanceHandler), typeof(ParentTrackingHandler))]`. The second constraint is for the wait, not the stream: the topology trigger evaluates branch scope from `GetParents()`, so a monitor running before `ParentTrackingHandler` would re-evaluate a pending wait against stale parents, decide the reparent had not made it satisfiable, and never look again. That is the missed wakeup the trigger exists to prevent. `ParentTrackingHandler` already runs before `ContextInheritanceHandler`, so the first constraint implies the second transitively today, but stating both means the guarantee does not silently evaporate if that attribute is ever removed. The attribute exists (`src/Namotion.Interceptor/Attributes/RunsAfterAttribute.cs`) and is already used in production (`PropertyChangeInterceptor` carries `[RunsAfter(typeof(SubjectTransactionInterceptor))]`), and `ParentTrackingHandler` solves the mirror-image problem with `[RunsBefore(typeof(ContextInheritanceHandler))]`. That existing constraint also means parents are already fresh when the wait's topology trigger fires.
...
- A stopped source stays stopped **because of the `StartAsync` guard**, not because the platform prevents it: `BackgroundService` would run `ExecuteAsync` again. `HostedServiceHandler` additionally drops the service from the subject's attached list on detach, so a re-attached subject is normally served by a new source instance that registers on its own. That covers most paths but not all, which is why the guard exists: a subject that is itself an `IHostedService` is re-added and restarted automatically on re-attach (`HostedServiceHandler.cs:28-33`), and an application holding a source instance can call `AttachHostedService(sameInstance)` explicitly.
...
- Waiting on a subject that is not attached to the tree usually **throws**, not blocks. `ContextInheritanceHandler` removes the parent fallback on the last detach and a never-attached subject never had one, so no monitor is resolvable from its context and the no-monitor rule applies. The block-and-warn behaviour is reachable only for an anchor that shares the tree root's context directly, where a monitor still resolves; such an anchor matches no source rooted elsewhere but still self-matches a source rooted on it.
- A wait already pending when its anchor detaches is a different case, because it captured its monitors at start. Its anchor's own source is stopped by the detach but stays registered until disposed, so it self-matches, every in-scope source is `Stopped`, and the wait completes vacuously with the warning. A wait *started* after that detach throws instead.
...
- `Namotion.Interceptor.Connectors`: `SourceState`, `SourceEventKind` (seven kinds, including the two view kinds), `SourceEvent` (including `CurrentState`), `ISubjectSource` members (`State`, `LastSynchronizedAt`, `PendingWriteCount`, `StateChanged`), `SubjectPropertyWriter`-driven transitions on `StartBuffering` and `LoadInitialStateAndResumeAsync` plus the internal transition interface it calls, `SubjectSourceBase` lifecycle transitions (pump entry, catch, finally, disposal fallback) serialized under a per-source lock, the `StartAsync` guard that refuses to restart a `Stopped` source, and `StartAsync` registration, `SourceMonitor` and `SourceSubscription` (including the lifecycle attach and detach catch-up, per-subscriber delivery queues, the registration count, and internal wait bookkeeping), the `WithSourceMonitoring()` and `WithSourceMonitoring(IServiceCollection)` context extensions and the hosted registration-hold service, the `GetSourceMonitor()`, `CompleteSourceRegistration()` and `DeferWaitCompletion()` context extensions, the `WaitForSynchronizationAsync` subject extension and its branch-scope helper, `GetSourceState` property extension, emission logic in `SetSource`/`RemoveSource` (built on `TryAddPropertyData`).
- `Namotion.Interceptor.Tracking`: `SubjectPropertyChange.GetCurrentValue<T>()`, the property-change mirror of `SourceEvent.CurrentState`, and `IPendingActionsBarrier` in the `Lifecycle` namespace, the seam letting the registration holder await queued lifecycle work without Connectors referencing Hosting; the Tracking public API snapshot updates accordingly.
- `Namotion.Interceptor.Hosting`: `HostedServiceHandler.WaitForPendingActionsAsync(CancellationToken)` (internal, a marker action posted onto the existing `BufferBlock`) and the public `WaitForPendingHostedServiceActionsAsync` context extension that reaches it. `Namotion.Interceptor.Hosting.Tests` has no `VerifyChecksTests.PublicApi` snapshot today, so there is none to update; adding one is worthwhile but is not part of this change.
...
- Every direct `ISubjectSource` implementer gains the four new members. That is the test doubles `ConcurrentTestSource` and `BlockingTestSource` in `Namotion.Interceptor.Connectors.Tests`, and also `BenchmarkSource` in `src/Namotion.Interceptor.Benchmark/SubjectTransactionBenchmark.cs:108`, which is production-adjacent code that fails the build if it is missed.
...
- Pump transitions via a fake source: `Connecting` initially; `Synchronized` after initial load; back to `Connecting` on pump failure; `Stopped` on cancellation; `LastSynchronizedAt` set exactly on entering `Synchronized` and preserved through a later disconnect; `Stopped` is terminal by guard: after a stop, a second `StartAsync` on the same instance does not launch the pump, does not transition, does not claim and does not synchronize, and the source stays `Stopped`. This test must exercise the real `BackgroundService` lifecycle rather than a stub, because the platform would otherwise happily run `ExecuteAsync` a second time.
...
- Branch scope: a source rooted at an ancestor of the anchor is in scope; a source rooted at a descendant is in scope; a source rooted on a sibling branch is excluded and its state never affects the wait; a multi-parent subject is in scope of sources on either parent path; an anchor with no parents matches no source rooted elsewhere but still self-matches a source rooted on it; a wait pending when its anchor detaches completes vacuously with a warning, while a wait started after that detach throws for lack of a reachable monitor; a same-tree reparent of the anchor re-evaluates a pending wait.
