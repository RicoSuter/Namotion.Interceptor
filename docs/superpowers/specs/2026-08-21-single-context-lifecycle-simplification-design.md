...
5. **Transient races block; only persistent conflicts throw.** The original text conflates the two, and the implementation threw for both. A subject genuinely owned by another context is a programming error and throws. A concurrent attach arriving while a structural write is in flight is a timing detail, not a caller error, and must not surface as an exception. The structural write therefore takes the lifecycle gate and the executor's attachment monitor **before** resolving the interceptor chain and holds them through the terminal, so a transition arriving while the write is in flight waits on the monitor instead of invalidating the write. A transition that lands between the lock-free routing read and the lock acquisitions is handled by revalidating under both locks, releasing them and retrying against the fresh attachment; the retry's progress properties (livelock-free, deliberately not starvation-free) are recorded in `docs/design/tracking-lifecycle.md`. This also removes a failure mode observed in the spike, where an unhandled rejection on a raw thread terminated the host process.

6. **The unattached structural write hole is closed, by the concurrency stage rather than by the attachment stage.** The original wording said structural setters publish an executor unconditionally so the guard always runs, and priced that at one executor allocation. Implementing it showed the price is higher and the timing is wrong.

   Higher, because the scalar helper's short circuit tests the same `_context` field the structural helper would populate. Once any structural write publishes an executor, every later write on the still unattached subject stops short circuiting too, so it takes a `SyncRoot` lock, an attachment monitor, a commit revision, a timestamp fetch and a write-state store. Measured on the `Person` fixture, whose parameterless constructor assigns a collection and then two scalars: `new Person(context)` consumed three commit revisions and stamped three write timestamps before ever being attached, against none on master. That surfaced as 36 connector `SubjectUpdate` snapshots gaining timestamps on construction-written properties, a queue test dropping both of its changes because the committed revision no longer starts near zero, and a batch test seeing 409 timestamp lookups against 100 mutations. The whole cost lands on construction and deserialization, which is where the two open cost questions already sit.

   Wrong timing, because until the exact context is authoritative the guard protects nothing: a subject attached through the old fallback path reports a null exact context, so `AttachedContext` cannot be the short-circuit predicate and `_context is null` is the only available proxy. Paying the cost in the attachment stage buys no correctness there.

   So the attachment stage keeps the short circuit, and the concurrency stage closes the hole the cheap way: the structural write takes the executor's attachment monitor before resolving the chain, and short circuits *inside* it when there is no attached context. A concurrent attach has to take that same monitor, so the race is closed, and the unattached path stays as cheap as master's. This is the same monitor decision 5 already requires, so it costs no new mechanism.

7. **Subtree-scoped subject-local services are removed**, deliberately and as a named breaking change rather than as collateral. Implemented in the ownership stage rather than the removal stage: the composition that produced subtree scoping is the same one that let a detached subject keep resolving the graph's services, made resolution as deep as the graph, and closed a resolution loop whenever two subjects each became the other's first parent, so removing it there retires all of those at once. Preserving them would require retaining the executor-as-context relationship, the fallback graph, delegation resolution, cycle detection and cross-context invalidation, measured at about 670 of the 1088 lines of `InterceptorSubjectContext.cs`, which is the entire simplification. If subtree scoping is wanted later it returns through the separately designed mechanism already listed under deferred extensions.
...
11. **Detach callback order flips for handlers behind the descent, and that is a fix.** Master has two detach orders, not one: a handler ahead of the descent sees `top, mid, leaf` while a handler behind it sees `leaf, mid, top`. Both exist only because `ContextInheritanceHandler` re-enters the detach and re-reads the backing store, running the group behind it a second time from the bottom. Merging that handler leaves one release traversal and therefore one order, so one of the two groups necessarily moves; keeping both would mean keeping two passes, which rebuilds the machinery the merge deletes. Top-down is chosen, which retains the documented, snapshot-pinned order of the ahead group and moves the undocumented order of the behind group. Measured on both arms with a three-level chain: attach order is unchanged (`leaf, mid, top`), and only detach flips, from `leaf, mid, top` to `top, mid, leaf`. Master starts deepest-first and stops deepest-first, which is not LIFO; the new order makes stop the reverse of start, so a parent stops before the children it uses rather than after them. Nothing documented breaks, because `docs/design/tracking-lifecycle.md` documents attach order only and its detach paragraph is about registry resolvability. There is no first-party hosted subject nested inside another, so no in-repo behaviour changes today, and the hosted-service ownership rework replaces the single global dispatch queue with one serialized chain per target, which removes cross-target stop ordering altogether. That rework rebases on top of this change and inherits the question as an overlap question rather than an order question.

12. **Structural writes from lifecycle callbacks are forbidden, as a declared breaking change.** Master permits them for a different property and the contract is documented at the top of the current reconciliation loop. The new design forbids all of them. Treated as a contract violation detected by `CallbackReentrancyGuard`, which is live in every build; see the 2026-08-24 lifecycle callback contract design for the settled rule.
