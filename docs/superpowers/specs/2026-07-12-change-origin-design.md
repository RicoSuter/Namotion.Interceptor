...
`SubjectSourceBase` currently creates its `ChangeQueueProcessor` subscription only after `LoadInitialStateAndResumeAsync`, and `PropertyChangeQueue` drops changes when no subscription exists. Any locally computed write-back produced during initial load (a hook transforming an inbound initial value, PR 1; an equality-suppressed correction, PR 2) is therefore silently discarded and the source stays diverged until it next pushes that property. "Claim ownership" is not a discrete step in `SubjectSourceBase.ExecuteAsync` today: ownership scanning happens inside the connector-specific `StartListeningAsync` / `LoadInitialStateAsync`. The lifecycle changes to: create the processor subscription, start listening, load and replay initial state (which includes connector-specific ownership claiming), reapply retries, then start processing. The echo filter depends on ownership being established before processing starts, so the ownership work must complete inside the load step that precedes processing.

Subscribing before the load means the entire initial snapshot can accumulate in the pre-processing window. A single unbounded FIFO queue (today's `PropertyChangeQueueSubscription._queue`) cannot express what this window needs: a bound cannot skip the oldest element of one FIFO, and a locally originated write-back that exceeds the bound would violate either the bound or the never-drop invariant. The window therefore segregates by origin. Changes whose origin has a source (`FromSource` and `Confirmed` echoes) go to a bounded, drop-oldest buffer; dropping them is safe because processing would skip them for their own source anyway. Locally originated changes (`Origin.Kind == Local`, including transform write-backs, which are rare during load) go to a small dedicated unbounded list that is never dropped, since losing one leaves the source diverged. When `ProcessAsync` starts, the local list is drained first, then the bounded buffer, whose ordinary initial `FromSource` changes are echo-dropped. The memory test asserts the bound on the echo buffer for a large initial snapshot and that no local write-back is ever lost. This segregation does not exist today and needs a new mechanism; if implementation shows it is invasive it may alternatively be descoped into its own PR, since the rest of PR 1 does not depend on it.
...
- Initial-load write-back: a hook-transformed inbound value applied during `LoadInitialStateAsync` reaches the source once processing starts (the subscription exists before the load); the pre-processing window segregates origins (a bounded, drop-oldest buffer for `FromSource` and `Confirmed` echoes, a never-dropped list for local write-backs) and the memory test asserts the echo buffer's bound for a large initial snapshot and that no local write-back is lost.
