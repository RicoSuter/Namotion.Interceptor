...
**Registration scope.** The `SourceCorrectionDetector` class lives in `Namotion.Interceptor.Tracking` because it needs internal access to `PropertyChangeQueue.EnqueueCorrection`, but its registration lives on the Connectors source-enabling configuration path (alongside `WithSourceTransactions` or whichever extension every connector setup already flows through), NOT the Tracking queue registration. A correction can only fire where a source exists, so an application with no connector setup registers no detector and pays zero PR 2 write-path cost; only source-enabled contexts carry the extra interceptor frame.
...
- PR 2 adds `SourceCorrectionDetector` as a `[RunsFirst]` write interceptor, but its registration lives on the Connectors source-enabling configuration path rather than the Tracking queue registration, so it is present only in source-enabled contexts. Where it is registered it executes on every write, including local ones: it fast-rejects on `Origin.Kind != FromSource`, but still adds one interceptor frame per write, and the PR 2 benchmark run covers this. Applications with no source (no connector setup) register no detector and pay zero PR 2 write-path cost.
