...
The fix for the `Local` population is a bounded cap on the subscription buffer during the pre-processing window: on overflow the oldest buffered change is dropped and a divergence warning is logged, naming `RequestResynchronization` / #342 as the remediation path. The bound makes worst-case load-window memory explicit and finite rather than proportional to snapshot fan-out, and a dropped write-back leaves the source diverged until the next resync, never a wrong model value. The memory test must drive a derived-heavy or hook-heavy model so buffered `Local` volume is actually exercised; a model with no derived or hook write-backs would leave the buffer empty and prove nothing.

Nothing correct is silently lost except under explicit, logged overflow. The enqueue filter drops only own-source echoes (which the dequeue skip would drop anyway); the bounded cap drops oldest only on overflow, always with a divergence warning. The WebSocket case, where the server stamps origins per connection while its processor identifies as the handler, never matches the filter, so per-connection inbound updates are buffered and broadcast rather than lost. The enqueue filter uses the exact same reference comparison as the dequeue skip, so a change is dropped at enqueue only when the dequeue skip would have dropped it anyway.
...
1. **An outcome exists** (a stamp was consumed, so the chain ran). A cancelled `OnXChanging` records nothing (the stamp was never consumed and no context was constructed), so it self-excludes structurally: no non-destructive peek of the pending slot is needed or added.
