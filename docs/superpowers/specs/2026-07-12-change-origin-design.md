...
Subscribing before the load raises one concern: the entire initial snapshot flows through the subject during the load window, and buffering that flood in a single FIFO subscription queue would spike memory. The fix is to apply the processor's own skip predicate at ENQUEUE time on the subscription instead of at dequeue. A change is droppable for a given subscription exactly when that subscription's processor would skip it at dequeue anyway: `ReferenceEquals(change.Origin.Source, processorSource)`. (In PR 2, `Correction` kinds bypass the skip and are always buffered, matching the dequeue rule.) The initial-load flood consists entirely of own-source echoes, because within `SubjectSourceBase` each inbound value is applied through `SetValueFromSource(this)`, which stamps the processor's own identity; those echoes match the enqueue filter and never enter the buffer at all. What is buffered during the load window is exactly what is buffered in steady state: local write-backs, changes from other sources, and (PR 2) corrections. The buffer therefore holds ordinary steady-state volume, not the snapshot flood, and the bounded-memory property holds with the existing single FIFO subscription queue.

Nothing unsafe is ever dropped. The enqueue filter drops only changes whose origin source is the processor's own source by reference; every non-matching origin is buffered, not dropped. The WebSocket case, where the server stamps origins per connection while its processor identifies as the handler, never matches the filter, so per-connection inbound updates are buffered and broadcast rather than lost. This resolves the identity-mismatch objection by construction: the enqueue filter uses the exact same reference comparison as the dequeue skip, so a change is dropped at enqueue only when the dequeue skip would have dropped it anyway.

Moving the own-source skip from dequeue to enqueue is behavior-identical for delivery, because the enqueue filter and the dequeue skip apply the same predicate, so the set of changes each processor writes to its source is unchanged. It is slightly cheaper in steady state, because own-source echoes are never buffered and never dequeued. The dequeue-time skip in `ProcessAsync` stays as a cheap defensive check.

The previous deferral rationale, that the reorder needs new buffering machinery and a delivery-based segregation out of scope for PR 1, is obsolete: enqueue-time filtering over the existing single FIFO subscription is sufficient, because the only changes that would flood the buffer are exactly the own-source echoes the filter already drops.
...
**Detection.** Correction detection lives in the Tracking primitive `SetValueFromOrigin`, which already brackets every stamped setter invocation: it sets the pending origin, enters the timestamp scope, invokes the setter, and disposes both scopes. After the setter returns, and only when `origin.Kind == FromSource`, `SetValueFromOrigin` decides whether the inbound value was silently suppressed. A dedicated write interceptor was rejected: it would add an interceptor frame to every write and require `[RunsFirst]`/`[RunsBefore]` ordering against the equality handler, whereas the arming wrapper already brackets exactly the one write in question and adds nothing to the write chain. The decision, taken while the pending scope is still open (before `Dispose` restores the previous frame), runs four guards:

1. **Timestamp moved.** Capture the property's raw write-timestamp metadata before invoking the setter and compare it after. A terminal write updates the write-timestamp; an equality-suppressed write never reaches it. If it moved, a real write landed and published through the change queue normally, so there is no correction.
2. **Stamp not consumed.** If the pending stamp is still armed for this property, the chain never started (an `OnXChanging` hook cancelled the write). Nothing was suppressed and there is nothing to correct, matching the previous detector, which never fired on a cancelled write. `PendingOrigin` exposes a small internal consumption-observation member (a non-destructive `IsArmedFor` peek reporting whether the frame `SetValueFromOrigin` set is still present) so the wrapper can tell a consumed chain from a cancelled one before the scope disposes.
3. **Active transaction.** If `SubjectTransaction.HasActiveTransaction` (the flag lives in the same assembly as `SetValueFromOrigin`), the value was captured as pending transaction state and commit replay reconciles it, so there is no correction. This is the transaction guard; it replaces the old projection-equality clause that used to exclude the transaction case structurally.
4. **Diverged equality suppression.** Otherwise (stamp consumed, timestamp unmoved, no transaction) the chain ran but nothing landed, which is exactly the equality check suppressing the write. Read the observable value; if it still equals the sent value there is no divergence (a pure echo or no-op) and no correction. If it differs from the sent value, the source's value was silently dropped and the correction is synthesized under the rules below.
...
- Local writes stay allocation-free: the pending stamp is thread-static fields, `ChangeOrigin` is a small struct, and `FinalizeOrigin` short-circuits on `Origin.Kind == Local` before it ever compares. The one caveat is stamped writes: the survival check calls `object.Equals` against the boxed `SentValue`, which boxes a value-type `NewValue` for the comparison. That boxing happens only on stamped inbound and cold paths, never on the local write path. In PR 2, correction detection reuses that same stamped path: its divergence check is an `object.Equals` over the already-boxed sent and observable values (no further boxing), so the boxing note applies there too. `SubjectPropertyChange` stores the origin flattened (kind byte plus source reference, see the model section), targeting no size growth versus master; a test measures `Unsafe.SizeOf<SubjectPropertyChange>()` and the benchmark run validates the outcome rather than assuming it. If no padding byte is free and the struct grows by one alignment slot (8 bytes) regardless, that growth is the accepted outcome, justified by the before/after benchmark on the PR rather than by further layout contortion.
...
- PR 2 adds ZERO write-chain cost: there is no correction interceptor, so the write chain is identical to PR 1 and every write, local or stamped, runs the same frames. Correction detection lives inside `SetValueFromOrigin` and runs only after `FromSource` (stamped inbound) applies, which are network-bound paths. Its added cost there is two raw write-timestamp reads plus a few comparisons; the divergence check is the `object.Equals` over already-boxed values noted above. An application with no source never performs a `FromSource` apply and pays zero PR 2 cost, with no registration to gate.
...
- Source lifecycle ordering (see "Source lifecycle ordering (PR 1)"): a hook-transformed inbound write-back produced during initial load reaches the source once processing starts, because the subscription now exists before the load; the initial-load flood does not grow the subscription buffer (a large simulated initial snapshot leaves the buffer empty, a bounded-memory assertion, because every own-source echo is filtered at enqueue); dequeue-time echo suppression is unchanged for steady state (delivery is behavior-identical to master).
...
- No spurious correction during transaction capture (inbound stamped write with an active transaction captures the value; no correction is emitted), pinning the `HasActiveTransaction` guard.
