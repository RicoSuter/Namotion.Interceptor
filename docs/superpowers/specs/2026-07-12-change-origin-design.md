...
`SubjectSourceBase` currently creates its `ChangeQueueProcessor` subscription only after `LoadInitialStateAndResumeAsync`, and `PropertyChangeQueue` drops changes when no subscription exists. Any locally computed write-back produced during initial load (a hook transforming an inbound initial value, PR 1; an equality-suppressed correction, PR 2) is therefore silently discarded and the source stays diverged until it next pushes that property. The lifecycle changes to: create the processor subscription, start listening, load and replay initial state, claim ownership, reapply retries, then start processing the buffered queue. Subscribing before the load means the entire initial snapshot can accumulate in the pre-processing window, so that window MUST use a bounded buffer rather than an unbounded per-subscription queue. The buffer applies drop-oldest semantics to `FromSource` changes only, which is safe because those are echoes that would be dropped for their own source anyway; locally originated write-backs are never dropped, since losing one leaves the source diverged. Ordinary initial `FromSource` changes buffered in that window are echo-dropped when processing starts. The memory test asserts the bound holds (the buffer does not grow without limit for a large initial snapshot), rather than merely observing that usage looks reasonable.
...
- Initial-load write-back: a hook-transformed inbound value applied during `LoadInitialStateAsync` reaches the source once processing starts (the subscription exists before the load); the pre-processing buffer is bounded (drop-oldest for `FromSource`, never for local write-backs) and the memory test asserts the bound for a large initial snapshot.
