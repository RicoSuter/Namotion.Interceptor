...
Derived-notification prerequisite (documented, not enforced at subscribe time). Automatic derived notifications require `DerivedPropertyChangeHandler`, which `WithDerivedPropertyChangeDetection()` registers and `WithFullPropertyTracking()` bundles, but which a bare `WithPropertyChangeNotifications()` context does NOT have. Without it, a dependency write never re-enters the chain to recalculate the derived value, so a subscription to a `[Derived]` property is accepted but stays silently inert. Subscribe cannot fail-fast on this (it requires no context, and the subject may attach later to a context that does have the handler), so `docs/tracking.md` and the `SubscribeToProperty` XML docs must state: derived-property subscriptions fire only when the subject's context also has derived-change detection (use `WithFullPropertyTracking()`), or when `RecalculateDerivedProperty()` is called manually (which re-enters the chain regardless).
...
- Derived subscription: with `WithDerivedPropertyChangeDetection()` (or `WithFullPropertyTracking()`) present, subscribing to a `[Derived]` property (non-partial, `IsIntercepted == false`, `IsDerived == true`) is accepted and fires when the derived value recalculates; on a bare `WithPropertyChangeNotifications()` context (no derived detection) the subscription is accepted but inert until `RecalculateDerivedProperty()` is called manually.
