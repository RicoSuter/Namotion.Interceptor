...
Date: 2026-07-16 (revised 2026-07-17: subject-stored registrations; second revision after external and independent adversarial review; third revision 2026-07-18: post-commit listener resolution with a Dekker fence pair, required by the path-subscription design in `2026-07-18-path-subscriptions-design.md` to close its missed-write window)
Target packages: `Namotion.Interceptor.Tracking` (primary) plus one additive internal member on a core struct, no public API change: a per-write dedup flag on `PropertyWriteContext` (core already grants `InternalsVisibleTo` to Tracking, `Namotion.Interceptor.csproj:17`)
...
{
    // Pre-commit gate: one volatile field read for the context facets, one static atomic read
    // for the process-wide listener hint. Both cache-hot. Decides whether the old value must
    // be captured; listener RESOLUTION is post-commit (below), so an install racing this
    // write cannot be missed.
    var state = _state;
    var mayHaveListeners = PropertyChangeSubscriptions.LiveCount != 0;
    if (state is null && !mayHaveListeners)
    {
        next(ref context);
        // Post-commit re-check (fence, count read, lookup): a subscription whose install
        // raced this write still gets delivered (Dekker pair, see Fast-path rules). The old
        // value was not captured on this path; a late-discovered listener receives a change
        // with OldValue equal to NewValue (documented caveat).
    ...
    // Listener resolution is POST-commit: full fence, count re-read, then the lookup on the
    // written subject's own small Data dictionary. The fence pairs with Subscribe's
    // increment-then-install (both interlocked), so any subscription installed before this
    // write's commit is seen here. Claimed during unwind: the innermost aggregated instance
    // resolves first; outer instances observe the claim and skip their lookup.
    PropertyChangeSubscription[]? listeners = null;
    Interlocked.MemoryBarrier();
    if (PropertyChangeSubscriptions.LiveCount != 0 && !context.ArePropertyListenersClaimed)
    {
...
// Idle-entry path only: full fence, count re-read, lookup, dispatch. Builds the change with
// the final value as both old and new, because the pre-write value was not captured on the
// idle path. Kept out of the main body so the idle fast path stays small and inlinable.
private void DispatchLateListeners<TProperty>(ref PropertyWriteContext<TProperty> context) { ... }
```
...
- Three-tier gating. (1) Two cache-hot pre-commit loads decide old-value capture and exit early when nothing is subscribed anywhere, followed post-commit by one fence and count re-read that closes the install race. (2) The post-commit applicability gate exits without building when consumers exist but none applies to this write. (3) Otherwise build once and dispatch only to the applicable facets.
- Post-commit listener resolution. The listener lookup runs after `next` so it observes the `Data` dictionary as of commit time. Combined with the fence pair (below), this guarantees: any write whose innermost field write commits after a subscription's install is delivered to that subscription. The queue and observable facets intentionally keep their pre-`next` snapshot semantics (carve-out (b) is unchanged); the stronger guarantee is listener-only, because subject-stored listeners are the primitive that higher-level chained subscriptions (the path-subscription design) build their re-subscription correctness on.
...
The chain threads one context by ref end to end on the writing thread, so flag mutations are visible to all instances in program order; two instances can never both claim. Listener resolution happens post-commit during unwind, so the innermost aggregated instance resolves first and claims; outer instances, whose unwind code runs later on the same thread, observe the claim and skip their lookup. Re-entrant writes are unaffected by the flag for a simpler reason: a nested write is a new `PropertyWriteContext` with a fresh flag, and it SHOULD deliver, because it is a distinct change. The guarantee is exactly once per write context, not once per callback cascade. Queue and observable facets are unaffected: they are interceptor-owned, so aggregation intentionally fans out to each interceptor's own consumers, as today.

### Fast-path rules

- Idle gate is two cache-hot loads: the `_state` volatile field (context facets) and the process-wide listener count, a `static long` (`PropertyChangeSubscriptions.LiveCount`, name open) of subscriptions created and not yet disposed, maintained with `Interlocked.Increment`/`Decrement` on subscribe and dispose; the gate reads it atomically (`Interlocked.Read`/`Volatile.Read`, the 32-bit-safe pattern the existing write-timestamp holder already uses) and checks `!= 0`. It is a `long`, not an `int`, because the cumulative fire-and-forget increments never decrement, so an `int` would wrap at 2^32 and could momentarily read zero while a listener is still live, silently stopping delivery; 2^63 registrations is unreachable. On the net9.0/64-bit target the atomic long read is a single load, so the hot path is unchanged (`volatile` cannot be applied to a 64-bit field, hence the explicit atomic read). Post-commit re-check accounting: every write additionally executes, after `next`, one `Interlocked.MemoryBarrier()` followed by one `Volatile.Read` of the count (and only when that reads nonzero, the `Data` lookup). The fence-then-plain-read pattern is deliberate: `Interlocked.Read` on the shared count is a compare-exchange (an RMW that dirties the cache line and contends across writer cores), while `MemoryBarrier` is core-local and the `Volatile.Read` leaves the line shared. The fence is required for the Dekker pair (next bullet); a release-then-acquire sequence does not order the commit store before the count load on weak memory models. Master comparison: the default `WithFullPropertyTracking()` configuration pays two interceptor hops with one gate load each; the merged idle path is one hop, two pre-commit loads, plus the post-commit fence and load, which the benchmark gate must show is not measurable (the fence is roughly a store-buffer drain, tens of cycles, on a path that already executed a lock acquire and release inside `next`).
- The count restores the fast path when the feature stops being used. Subscribe does `Interlocked.Increment` before installing into `Data`; the one-shot disposal transition does `Interlocked.Decrement` after removing from `Data`, so a concurrent write never reads zero while a subscription is still installed. When every per-property subscription in the process is disposed, `LiveCount` returns to zero and writes take the zero-lookup fast path again. One honest caveat, documented: a fire-and-forgotten subscription (handle and subject dropped without `Dispose`) never decrements, so its slot persists until process exit and keeps the lookup active. This is not a new cost (a monotonic flag would have stayed set forever regardless); it is the reason to prefer disposal when you want the fast path back. A finalizer could reclaim fire-and-forget slots on GC but was rejected: finalizing every subscription adds allocation-time and extra-GC-generation overhead (against priority 2) to buy back one hot-`Data` probe per write, and finalizers for pure bookkeeping are an anti-pattern. While the count is nonzero, every intercepted write pays one lookup on the written subject's own small `Data` dictionary (usually a miss); this active-lookup state is a benchmarked configuration. Delivery guarantee (the Dekker pair): `Subscribe` performs `Interlocked.Increment` on the count (a full fence) and then the interlocked install into `Data`; the write path commits, executes `Interlocked.MemoryBarrier()`, re-reads the count, and resolves listeners. Store-buffering reasoning then gives at least one of two outcomes for a racing pair: the write's post-commit lookup sees the install (and delivers), or the subscriber's first post-install read of the property sees the committed value. Consequently a write that commits after the install is always delivered; only a write that commits before the install can go undelivered, and its effect is visible to the subscriber by reading the property after `Subscribe` returns. One caveat, documented: a write that entered on the idle path (count was zero at its pre-commit gate) and discovers a listener at the post-commit re-check delivers a change with `OldValue` equal to `NewValue`, because the pre-write value was not captured. This guarantee is what the path-subscription design's subscribe-before-read rebuild discipline relies on; without post-commit resolution, a writer whose lookup ran before an install but whose commit landed after the subscriber's read would dispatch to nobody.
...
Derived-notification prerequisite (documented, not enforced at subscribe time). Automatic derived notifications require `DerivedPropertyChangeHandler`, which `WithDerivedPropertyChangeDetection()` registers and `WithFullPropertyTracking()` bundles, but which a bare `WithPropertyChangeNotifications()` context does NOT have. Without it, a dependency write never re-enters the chain to recalculate the derived value, so a subscription to a `[Derived]` property is accepted but stays silently inert. Subscribe cannot fail-fast on this (it requires no context, and the subject may attach later to a context that does have the handler), so `docs/tracking.md` and the `SubscribeToProperty` XML docs must state: derived-property subscriptions fire only when the subject's context has derived-change detection (use `WithFullPropertyTracking()`). Manual `RecalculateDerivedProperty()` is NOT an escape hatch: it too is a no-op without derived detection, because it early-returns when the property has no `DerivedPropertyData` (`DerivedPropertyChangeHandlerExtensions.cs:44-47`), and that data is created only by the derived handler's `AttachProperty` (`DerivedPropertyChangeHandler.cs:62-65`), which runs only when `WithDerivedPropertyChangeDetection()` is registered. So on a bare notifications context a derived-property subscription is simply inert.
...
The strongly-typed resolver is Tracking-local and strict about expression shape: unwrap any `Convert` boxing node, then require the body to be a `MemberExpression` whose `.Member is PropertyInfo` and whose `.Expression` is the lambda's own `ParameterExpression`, take `.Member.Name`, and build `new PropertyReference(subject, name)`. The `PropertyInfo` check rejects field (and other non-property) selectors: a field selector would still bind by name, so a base-typed selector `b => b.SomeField` whose name is reused by an intercepted property on the runtime subject (possible via `new`-hiding across member kinds) would silently subscribe to that unrelated property, the same wrong-property failure the strict shape check exists to prevent. The parameter-target check is essential: `PropertyReference` accepts any (subject, name) pair and validates the name only lazily, so without it a chained selector `m => m.Child.Speed` or a captured-variable selector `m => other.Speed` would resolve to the leaf name `Speed` and, if the root subject also has a `Speed`, silently subscribe to the wrong property. Chained, captured-variable, and static-member selectors therefore throw a clear error; only direct `m => m.Foo` is supported. The Registry's `TryGetRegisteredProperty` resolver is not used because Registry depends on Tracking (package cycle).
...
New per-property tests (`When<Condition>_Then<ExpectedBehavior>` naming, Arrange/Act/Assert). Static-state isolation: because `LiveCount` is process-wide and xUnit runs different collections in parallel within an assembly, EVERY test that creates a per-property subscription (not only those that assert on the count or fast path) must run in a single dedicated, non-parallel collection, or assembly parallelization must be disabled for the Tracking test project; otherwise a `Subscribe` in a parallel collection bumps the shared count and poisons the fast-path and count assertions. Those tests also use an internal reset hook for the count (Tracking grants `InternalsVisibleTo` to its test project).
...
- Install-during-write (post-commit resolution): an orchestrated test where a subscription is installed after a write has passed its pre-commit gate but before its commit; the write must deliver to that subscription. On the idle-entry variant (count was zero at the writer's gate) the delivered change carries `OldValue` equal to `NewValue` (pins `DispatchLateListeners` and the fence pair). A companion test pins the inverse: a write that commits before the install may be missed, but the subscriber reading the property after `Subscribe` observes the committed value.
...
- Strongly-typed `SubscribeToProperty(x => x.Foo, ...)` resolves the correct property; an unknown member throws; a member that is neither intercepted nor derived throws; a chained selector (`x => x.Child.Value`), a captured-variable selector, a static-member selector, and a field selector throw rather than silently binding the leaf name (only property members are accepted).
- Derived subscription: with `WithDerivedPropertyChangeDetection()` (or `WithFullPropertyTracking()`) present, subscribing to a `[Derived]` property (non-partial, `IsIntercepted == false`, `IsDerived == true`) is accepted and fires when the derived value recalculates; on a bare `WithPropertyChangeNotifications()` context (no derived detection) the subscription is accepted but fully inert, and manual `RecalculateDerivedProperty()` is also a no-op (it requires derived detection to have attached `DerivedPropertyData`).
...
- Benchmark the write path in `Namotion.Interceptor.Benchmark` for: idle (which now includes the post-commit fence and count re-read), queue-only active, observable-only active, both-active, listener-on-written-property, listener-elsewhere (count nonzero, lookup miss), and observable-subscribed-then-fully-unsubscribed, before and after, proving no regression and confirming the build-once improvement. The idle configuration is the gate for the post-commit re-check cost specifically.
