...
- Disposal is a one-shot atomic transition: `Interlocked.Exchange` on `_disposed`; only the first transition performs the copy-on-write removal and then clears the subscription's subject and observer references with `Volatile.Write` (pairing with the dispatch-side `Volatile.Read`), so a disposed handle pins neither. Repeated or concurrent disposal is a no-op.
...
Accepted deviation, documented: if a consumer deliberately built the old split topology (observable-only parent, queue-only child), the child now inherits the parent's dual-facet interceptor instead of getting its own, so a queue subscription created on the child would observe the parent's writes too. No product code does this, and queue consumers filter by source/property (`ChangeQueueProcessor`), so the surplus is filtered out rather than misdelivered. Listed alongside the other carve-outs.
