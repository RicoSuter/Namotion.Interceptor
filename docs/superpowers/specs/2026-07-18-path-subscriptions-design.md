...
Depends on: the per-property subscription primitive from `docs/superpowers/specs/2026-07-16-property-change-interceptor-design.md`, now implemented and settled on this branch: `PropertyChangeInterceptor` (dual-channel queue plus observable), `PropertyChangeSubscription` with `Subscribe`/`SubscribeToProperty` extensions, `WithPropertyChangeSubscriptions()` registration, and the post-commit listener resolution with the Dekker fence pair this design requires (implemented in `PropertyChangeInterceptor.WriteProperty` and `DispatchLateListeners`, gated by `PropertyChangeSubscriptions.ReadSubscriptionCount()`). This feature composes that surface and adds no core API.
