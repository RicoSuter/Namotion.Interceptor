...
- Getter-throw carve-out: a getter that throws on one walk publishes unresolved; a write only to the off-path condition the getter reads does NOT heal the event stream (it is not a watched segment), while `Current` already reflects truth; a subsequent watched-segment write re-walks and heals (pins that recovery needs a delivered segment callback, not the condition clearing).
