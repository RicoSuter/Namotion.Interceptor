...
- Escalation path if the validating walk ever measures as a problem at realistic depths: weaken to validating only on structural events (leaf deliveries read just the leaf), which trades away the dormant-divergence healing on leaf writes and must then be documented as a consistency carve-out. Not taken preemptively; the benchmark decides.
- Zero-cost-when-unused needs no dedicated configuration: path subscriptions are ordinary per-property listeners composed by the tracker (no additional interceptor, no new hot-path state), so subjects without path subscriptions are covered by the primitive's own idle and listener-elsewhere benchmark gates.
...
- Cost model, stated in one or two sentences so consumers can judge the price before adopting the feature. Wording, roughly: "A path subscription costs nothing on writes outside the watched path. A write to a watched segment pays, inline on the writing thread, a short per-subscription lock, a validating walk along the path (one property read per segment), and your callback; structural changes additionally pay the re-subscription of the segments below the change."
