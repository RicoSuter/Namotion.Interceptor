...
Depends on: the per-property subscription primitive from `docs/superpowers/specs/2026-07-16-property-change-interceptor-design.md` (branch `feature/property-change-interceptor`), specifically its third revision (post-commit listener resolution with the Dekker fence pair), which this design requires for the missed-write closure below. This feature must land after that work; it adds no core API and composes the primitive.
...
{
    ValueChange,   // the resolved leaf property itself was written; Cause is that leaf write
    PathChange     // an intermediate segment was written and the observed state changed
}
...
The tracker decomposes the expression into an ordered segment list and maintains one per-property primitive subscription per segment on the currently resolved subject: `Foo` on the root, `Bar` on `Foo`'s subject, `Baz` on the subject at `Bar[3]`. Value reads during walks use the subject's property metadata getters; shape checks use `SubjectPropertyTypeExtensions`. No Registry types are referenced.

- Leaf segment fires: deliver a `ValueChange` (subject to transition suppression, below).
- Intermediate segment fires: dispose the suffix subscriptions below the changed segment, rebuild the suffix top-down, and deliver a `PathChange` only if the observed state changed.

Rebuild order is subscribe-before-read, per segment, under the tracker lock: for each segment below the change, first subscribe that segment's property on the resolved subject, then read the property value (indexing into it where applicable) to resolve the next subject. Correctness rests on two halves. First, the primitive's post-commit listener resolution (the 2026-07-16 spec's third revision, added for exactly this design): any write that commits after a subscription's install is delivered, because the writer resolves listeners after its commit behind a full fence. Second, the subscribe-before-read order: a write racing the just-subscribed segment either commits before the tracker's read (the read sees the new value and the walk continues on the true graph) or commits after the install (the primitive then guarantees dispatch, which serializes on the tracker lock into a fresh retrack). Together the chain can never keep watching a subject the graph has already left. The reverse order (read values first, subscribe afterwards) leaves a window where a replacement of an already-read segment goes undispatched, detaching the chain from the graph until the next write to that same property; tests pin the racing interleavings, including the writer-lookup-before-install, commit-after-read variant that tracker-lock serialization alone cannot close. The same discipline applies to the initial subscribe-time build.
...
- `New` is a fresh read of the leaf (or unresolved). It is not copied from the causing write's payload.
...
Caveat, documented: with `WithEqualityCheck` active, replacing an intermediate subject with an instance that compares equal via `EqualityComparer<T>.Default` (a subject type overriding `Equals`) is filtered before interceptors run, so no structural event fires and the chain keeps watching the old instance while `Current` (a fresh walk) reports the new one. Subjects normally do not override `Equals`; this is documented, not solved.
...
- Quiescent consistency: once writes settle, the last event's `New` and `Current` agree with the real graph. The guarantee is scoped to non-throwing callbacks (see next point).
...
- `Cause.Origin` lets a connector distinguish Local from FromSource writes even for structural events (echo suppression).
- `Cause.Property` names the segment that moved; for a `PathChange` its old/new values are the intermediate property's values (for example the old and new collection references).
...
- A subject moved off the path has its segment subscription disposed during the retrack that the move itself triggers, so there are no stale pins in steady state. A structural move that happens while a segment is dormant leaves that segment's stale subscription (and its pin) in place until the next observed write at or above the stale point, or handle disposal (documented, see dormancy).
- Each segment subscription increments the primitive's process-wide `LiveCount`, so an active path subscription keeps the per-write listener lookup active, exactly as N individual per-property subscriptions would.

### Dormancy (inherited from the primitive)

A segment fires only while its subject is attached to a context with a `PropertyChangeInterceptor`. Structural changes made while a segment's subject is dormant are not observed, and the chain re-syncs at the next delivered write at or above the stale segment. `Current` is always computed fresh, so reads never go stale even while events are dormant. Subscribing before the root is first attached is valid; the subscription is dormant until attach. This is the primitive's documented dormancy caveat in path form and is documented, not solved with machinery (a `Refresh()` re-sync hook is listed under future extensions).

Functional prerequisite, documented prominently: multi-segment paths require context inheritance. `WithPropertyChangeNotifications()` alone registers only the interceptor; without `ContextInheritanceHandler` (registered by `WithContextInheritance()` and bundled by `WithFullPropertyTracking()`), a child subject assigned into the graph never inherits the root's context, so its writes bypass interception forever: segment 0 fires, every deeper segment is silently inert, `Current` still works, and nothing errors. For the primitive this is a user-controlled dormancy edge; for paths, whose purpose is observing subjects that appear dynamically, it is a prerequisite. `docs/tracking.md` and the `SubscribeToPath` XML docs must state: use `WithFullPropertyTracking()` (or at least `WithPropertyChangeNotifications()` plus `WithContextInheritance()`) for paths spanning more than one subject. A test pins the bare-notifications configuration as inert below the root.
...
Naming per repo convention (`When<Condition>_Then<ExpectedBehavior>`), Arrange/Act/Assert comments. All tests creating subscriptions run inside the same non-parallel collection discipline the primitive's tests use (process-wide `LiveCount`).
...
No stale subscriptions (the chain invariant). After every structural change, the multiset of segment listeners installed across subjects matches exactly the current resolved chain's segments, nothing more (a subject appearing at two positions of a cyclic path holds two listeners):

- Replacing an intermediate subject disposes the old suffix's listener entries: the replaced subject and everything below it hold no listener afterwards (white-box `Data` assertion), and `LiveCount` equals the current chain length.
...
- Repeated structural churn (replace, break, heal in a loop) never grows `LiveCount` beyond the current chain length (leak check).
...
- Cyclic path (the same subject and property at two segment positions, for example `x => x.Child.Child.Name` on a self-referencing graph): one write fires both segment observers, slot identity keeps them distinct, the chain invariant holds per segment.
...
- Bare `WithPropertyChangeNotifications()` context (no context inheritance): a multi-segment path is inert below the root (pins the documented prerequisite); with `WithFullPropertyTracking()` the same scenario delivers.
...
- Dispose stops delivery, tears down all segment `Data` entries, and returns `LiveCount` to zero; double dispose is a no-op.
- Fire-and-forget: dropped root plus dropped handle is collectable (GC test, as for the primitive).
- Dormancy: no delivery while the root is detached; delivery resumes on re-attach; a structural change made while dormant is missed and the chain re-syncs on the next observed write above the stale point (pins the documented caveat); `Current` stays fresh throughout.
...
- `Cause` correctness: kind, moved segment, and origin passthrough (FromSource writes keep their origin on structural events).

Docs:

- New section in `docs/tracking.md`: path subscriptions, observed-state model, delivery contract, dormancy, ownership, transaction visibility.
...
Resolved 2026-07-18: the missed-write window is closed by amending the primitive (option 1 of the previously blocking question). The primitive's spec (third revision) and its implementation plan now specify post-commit listener resolution with a Dekker fence pair: listeners are resolved after `next` behind `Interlocked.MemoryBarrier()`, the aggregation claim happens during unwind, and the idle fast path re-checks the count post-commit (`DispatchLateListeners`, with the documented `OldValue` equal to `NewValue` caveat for a write that raced the install). The tracker's subscribe-before-read discipline plus that guarantee close both racing interleavings; no verification re-read loop is needed. No blocking decisions remain.
