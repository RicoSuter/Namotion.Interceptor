...
Documented seam: subscribe-then-read-`Current` is not atomic with the event stream. A write may fire the callback before the consumer reads `Current`. Pull consumers (render from `Current`, re-render on each callback) are always safe, since `Current` is authoritative. A caching consumer must not seed with a value captured before a newer callback applied, or it can go permanently stale (read `Current` A, a callback applies A to B, then the stale cached seed A is applied last, and with no further write the consumer stays at A). The safe pattern: subscribe first, then have the seed and every callback apply under one consumer lock, reading `Current` at apply time (`lock (gate) { state = subscription.Current; }`) rather than a cached snapshot or the event `New`; the last apply then always reads the freshest `Current`, and the guaranteed callback after the last write makes it converge. Delta-applying consumers need the ordering a commit sequence number or a `Refresh()` hook would give (issue #385, future extensions).
...
Walk failures resolve to unresolved rather than throwing into a writer's write chain: a runtime subject whose segment property is missing or fails the `IsIntercepted || IsDerived` check, any getter that throws during a walk (derived getters may throw; the derived machinery already treats that as an anticipated condition; a transient impure-getter throw is a quiescent-consistency carve-out, see the Delivery contract), and a leaf whose runtime value is not assignable to `TValue` (possible via `new`-hiding of a same-named property across types, or dynamic subjects) all mark the path unresolved from that segment; no `InvalidCastException` escapes into the writer's chain. After such a walk the tracker is consistent: the prefix stays subscribed, the suffix is torn down.
...
- Transient getter throw: a getter that throws on one walk publishes unresolved; a pure getter heals on the next write that clears the condition, and `Current` reflects truth even before the heal (pins the getter-throw carve-out).
...
- New section in `docs/tracking.md`: path subscriptions, observed-state model, delivery contract (including the quiescent-consistency carve-outs: throwing callbacks, dormant divergence, and a foreign synchronous observer throwing on a shared write, which aborts dispatch before the segment callback), dormancy and the context-inheritance prerequisite, ownership, transaction visibility.
