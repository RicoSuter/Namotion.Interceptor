...
Derived-intermediate carve-out: a child subject reachable only through a `[Derived]` intermediate (not held by any intercepted property anywhere in the graph) is not attached at graph attach, because `LifecycleInterceptor` discovers children only through intercepted properties (`FindSubjectsInProperties` skips non-intercepted metadata) and derived properties are emitted non-intercepted. Its segment subscriptions are installed but dormant until the derived property first recalculates through the write chain (which triggers the attach), and that recalculation happens only when the derived property's inputs change, so a stable derived intermediate leaves the suffix inert. The common case, a derived intermediate that aliases a child already held by an intercepted property (for example `Best => Candidates[0]`), is attached via that property and is unaffected. A test covers the exclusive-hold case.
...
- Derived-intermediate exclusive-hold: a derived subject-typed intermediate whose child is referenced by no intercepted property; subscribe, write the child's leaf before any recalculation, and assert no delivery; then change a derived input to force recalculation and assert the suffix attaches, heals, and subsequently delivers.
...
- New section in `docs/tracking.md`: path subscriptions, observed-state model, delivery contract (including the quiescent-consistency carve-outs: throwing callbacks, dormant divergence, a foreign synchronous observer throwing on a shared write which aborts dispatch before the segment callback, and a getter throwing during the walk which recovers only on a later delivered segment callback), dormancy and the context-inheritance prerequisite, ownership, transaction visibility.
