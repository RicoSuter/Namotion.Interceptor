...
public readonly struct SubjectPathValue<TValue>           // one observed state of the path
{
    ...
    public TValue GetValueOrDefault(TValue fallback);     // fallback ONLY when unresolved; a resolved null leaf returns null, not the fallback
}
