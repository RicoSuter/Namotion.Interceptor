...
To keep the non-disposal from being "fixed" later by a maintainer or an analyzer (a disposable type holding an undisposed disposable field reads like a bug), the `WriteLock` property in `SubjectSourceBase` carries a short comment stating that it is intentionally not disposed: no wait handle is ever created, and disposing would race in-flight writes. The interface contract states the same for direct implementers: the XML docs on `ISubjectSource.WriteLock` explicitly forbid disposing the semaphore, since "owned by the source" would otherwise conventionally invite disposing it with a disposable source.
...
- `Namotion.Interceptor.Connectors.Tests/SubjectSourceExtensionsTests.cs`: fake sources implement `WriteLock`; `ConcurrentTestSource` drops the marker interface and returns `null`; the mock-based serialization test gains an explicit `WriteLock` setup and is rewritten deterministically (gate `TaskCompletionSource` instead of a 50 ms overlap window).
...
- Existing tests: write serialization (max 1 concurrent), concurrent opt-out (multiple concurrent), cancellation during semaphore wait. These cover the new code paths; beyond the fake-source member additions, the serialization test is made deterministic (a gate `TaskCompletionSource` replaces its 50 ms overlap window, which could only miss a broken lock).
- New tests: a `SubjectSourceBase`-derived source with the default `WriteLock` gets serialized writes, and one overriding `WriteLock` to `null` gets concurrent writes (replaces the marker-interface variant of that guarantee). Both use the same deterministic gate pattern: async methods run synchronously to their first incomplete await, so entry counts at task-creation time distinguish a working lock from a missing one without any timing.
