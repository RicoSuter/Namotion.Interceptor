...
The genuinely in-flight window (an inbound value from the same source lands while the assertion write is already on the wire, and the inbound change is echo-skipped) is **accepted at parity with ordinary writes** (decided 2026-07-16). It is an instance of a limitation that exists today for every outbound write racing an inbound apply from the same source, and it is currently undocumented on master (the limitation text exists only in the #372 branch's unmerged docs). Rationale: the correctness bar here is quiescent consistency, which the local-first sync model already relaxes for this window on all writes; a bounded post-write recheck would not restore it (it narrows the window without closing it) while readmitting the synthesized design's most complex machinery, an out-of-band follow-up write path; and the decision is asymmetric, since the recheck can be added later purely additively at the source-write layer if field experience demands it. Recovery is the connector reconciliation tier: read-after-write on OPC UA (on by default), state-digest divergence detection on WebSocket, the source's next inbound event, and explicit resynchronization via #342, which is the class-level fix.
...
9. Resolved 2026-07-16: the in-flight window is accepted at parity with ordinary writes (see the staleness section); the bounded post-write recheck remains available as a future additive hardening if field experience demands it.
