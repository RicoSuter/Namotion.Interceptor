...
Each structural terminal starts from the initiating property's current committed Tracking entry, or from the coordinator accumulator's most recent successful terminal overlay when downstream has already stored within the same coordinator invocation. The proposed direct value is visited once in collection order. Reference-identity lookup deduplicates Core membership while retaining the first proposed index or key for property projection. A retained child may update first-occurrence metadata without removing and re-adding its Core membership. Integer indices remain unboxed in an internal discriminated occurrence representation; conversion to `object` is permitted only when constructing an actual callback whose public contract requires it.

When the proposed value is the exact committed value object, direct mutation outside the gated setter remains unsupported, and no earlier terminal overlay supersedes that baseline, the planner takes an `O(1)` reference-identity fast path. Otherwise the diff records added, removed, retained, and first-occurrence-changed children in expected `O(old + new)` time. Inline zero/one entries avoid a lookup allocation. Larger committed entries own a lazy reference-identity index and nonsemantic operation visit token so warmed same-membership comparisons, including values larger than the pool retention cap, do not require a per-call set. An operation token and pending occurrence never become committed semantics. Nested work cancels the tentative token before using the entry, and finalization clears every pending occurrence reference and invalidates its numeric token before returning the frame.

One coordinator invocation owns one accumulator-local committed overlay. After each successful downstream terminal store and Core ledger commit, that terminal replaces the overlay for the initiating property and the accumulator's final committed delta. A later terminal diffs against that overlay rather than the pre-invocation persistent baseline. Cancellation clears only the currently tentative attempt and preserves any earlier successful overlay. Reconciliation applies only the final coalesced overlay to Tracking's persistent baseline before callbacks. An upstream repeated continuation creates a separate coordinator invocation and therefore a separate overlay each time.
...
For each directly added child, Tracking first asks Core to validate and reserve the exact incoming membership. A subject already committed in the same exact ownership domain and coordinator is compatible; the planner records the new incoming membership and stops recursive discovery at that subject because its committed outgoing baseline is already indexed. An owned subject from another domain or coordinator rejects before the backing store. A route-free subject is traversed through direct ownership readers. A pooled reference-identity visited index ensures each newly reached subject/property is discovered once while preserving cycle and DAG joins. Back and cross edges become incoming memberships but do not replace an already valid selected route.

Discovery validates the complete newly reached route-free subtree before commit and pins callbacks only for subjects it affects. It never walks through a compatible already-owned subject to rediscover its descendants. The cost of an addition is therefore the local old/new property diff plus newly reached subjects and edges, independent of already indexed compatible descendants and unrelated roots.

An explicit anchor remains selected while present. Otherwise the current selected incoming parent remains selected when that exact membership survives and remains acyclic. Removing or adding a nonselected compatible membership does not churn route selection. Only loss or invalidation of the selected relationship invokes insertion-ordered selection among surviving compatible acyclic parents. Ownership route transfer occurs only when the selected relationship or route target actually changes. First publication and PR #474 context-state invalidation retain their existing rules. The exact generation, expected-descriptor, and stale-no-op rules remain unchanged.
...
For such a trigger, the planner builds `C`, the forward closure from the potentially released subject or subjects through the prospective outgoing baseline, including the initiating-property or metadata-batch overlay. It seeds `live` with every subject in `C` that retains an explicit anchor or has a surviving prospective incoming membership from a subject outside `C`, then follows prospective outgoing edges from those seeds within `C`. The final-release set is `C - live`. Core removes incoming memberships whose parents are released and reserves final release for every subject in that set. This releases an orphaned cycle, preserves a shared descendant reached from a live seed, and handles a new outside edge in the same operation in expected `O(C + EC)` time. It never inspects an unrelated root or component.

The planner computes affected pre-order and post-order once in pooled storage. Affected subjects are direct membership additions/removals, first-attached subjects in newly reached subtrees, final-detached subjects in `C - live`, selected-route transfers, and a property owner whose retained occurrence projection changes. Only those subjects pin current removal arrays, prospective addition/transfer arrays, property arrays, generations, and route descriptors. Dispatch remains linear in the affected set, detaches an old child completely before attaching a replacement, invokes each required pre/post phase once, and publishes routes only at the exact coordinator slot. No callback order, visibility, route-selection result, generation rule, or stale-tail rule changes.

### Frames, lookup, and retention

Every Core structural prefix scope and coordinator accumulator owns a distinct LIFO pooled carrier. Every Tracking coordinator invocation owns a distinct LIFO frame identified by its exact Core batch. A synchronously nested coordinator invocation rents another Core accumulator and Tracking frame even while an outer frame is preparing. Cancelling the outer tentative attempt clears only its reservations, pending visit tokens, and proposed overlay; it cannot clear, reuse, or roll back the nested frame or an earlier successful terminal overlay.

Affected entries, direct child entries, memberships, and traversal work use inline-first storage. Reference-identity dictionaries and executor-to-entry indexes are created lazily only after the inline threshold and are retained in thread-static pools. Core and Tracking clear every subject, property, key, descriptor, handler array, previous-frame link, and other application reference before returning a frame or buffer. Retained list capacity and set/dictionary capacity-equivalent are capped at 1024. Storage exceeding 1024 is cleared and dropped instead of being retained. Large warmed stable collections use their persistent per-property identity index and visit tokens, not a newly allocated oversized operation set.
...
| new collection value | expected `O(old + new)` |
...
4. At each terminal, build `SubjectOwnershipWriteContext<TProperty>` from that attempt's final transformed value. The lifecycle coordinator performs the incremental initiating-property diff against the committed baseline or accumulator overlay, traverses only newly reached route-free subjects, runs an affected-component scan only when selected incoming removal can release ownership, and pins arrays only for the resulting affected subjects.
...
8. While retaining the topology gate, publish that attempt's preallocated incremental Core incoming-ledger entries under one affected executor monitor at a time, stamp its generation, and replace the coordinator accumulator's property overlay and final committed delta. Clear superseded attempt-only reservations without publishing callbacks.
...
22. Warm same-reference, same-membership same-order, and same-membership reorder writes. Planner counters prove the same-reference row performs constant work, reorder touches only the initiating property's old/new occurrences, integer indices remain unboxed until a real callback, and `GC.GetAllocatedBytesForCurrentThread` reports zero protocol bytes beyond the supplied value and configured application work.
23. For collection sizes 64, 128, 256, 512, 1024, and 2000, count proposed occurrences, reference-identity lookups, newly reached subject/edge visits, and possible-release component visits. One add and one disjoint replacement are bounded by a fixed linear function of old plus new plus newly reached or released edges. Cumulative growth has an `n^2`, not `n^3`, visit signature after accounting for input traversal.
24. Attach an unrelated owned graph at increasing sizes, warm it, then perform the same local stable write and local one-child replacement. Exact direct-reader, property-entry, identity-lookup, affected-subject, and callback-array counters remain unchanged as unrelated graph size grows.
25. During outer tentative discovery, synchronously enter a nested structural coordinator invocation. Exact frame/batch identities differ. Cancelling and releasing the outer attempt clears only its reservations and visit tokens; the nested committed overlay, persistent baseline, ledger, route, and frame cleanup remain intact.
26. Exercise `Root -> A -> B <-> C`, a shared DAG with an outside live seed, nonselected incoming removal, selected incoming removal, and final anchor removal. Forward-closure and live-seed counters touch only `C` and its edges, release `C - live` once in post-order, preserve shared live nodes and the surviving selected parent, and never inspect an unrelated root.
27. For an affected subtree beside a large compatible already-owned join, assert discovery stops at the join and pin/reconcile counters contain only direct membership changes, new/released subjects, transfers, and occurrence-refresh owners. Each required pre/post callback phase occurs once with the existing route/generation order.
28. Grow every Core and Tracking operation buffer past 1024, complete and release, then run a small nested operation and a warmed stable collection larger than 1024. Weak references prove oversized pooled buffers and cleared frame references are not retained; the next small frame is bounded and the large stable row remains allocation-free through its persistent property index and visit tokens.
...
- Core's incoming ownership ledger is indexed by subject executor and exact incoming `PropertyReference`; its explicit anchor, selected parent, route, domain, coordinator, and generation are the authoritative ownership truth. Core rents prefix and coordinator batches from a thread-static LIFO pool and never clones the ledger into them.
...
- Tracking's persistent outgoing baseline is indexed by subject and structural property, with inline-first unique child entries and lazy reference-identity lookup. It contains only committed structural values and first-occurrence projection metadata. It is not an ownership source and removes entries on final release.
...
- Warmed route-free and owned structural writes whose membership and effective route remain stable allocate zero managed bytes beyond configured application work. This includes same-reference values, same-membership new collection values and reorders, Core prefix/coordinator batches, Tracking frames, local lookup/visit state, affected-only pinning, and collections larger than the operation-pool retention cap.
...
- An actual membership change may allocate the new persistent Core incoming-membership state, Tracking outgoing property/child entry, or persistent overflow/reference index required to represent the committed topology. It may not allocate a rooted-graph snapshot, cloned ledger, fresh per-call operation object, boxed integer occurrence, or callback/service array for an unrelated subject. Replacement route objects are still constructed only after final validation and the exact stale generation/descriptor check.
...
- Static inspection must prove that a warmed structural path cannot construct a fresh batch/frame, clone a committed ledger, call `ToDictionary`/`ToArray` for planner state, scan all property entries for each subject, use a linear `Contains`/`IndexOf`/`FindSubject` after the inline threshold, enumerate every explicit root, or box an integer occurrence. Outgoing traversal starts from the indexed subject/property entry, reference lookup becomes lazy after inline storage, route state is absent before the exact publication phase, and every pool follows the 1024 clear/drop rule.
- Deterministic planner diagnostics report supplied occurrences, identity lookups, newly reached subjects/edges, possible-release subjects/edges, affected subjects, pinned arrays, and managed protocol bytes for the size series 64, 128, 256, 512, 1024, and 2000. Cumulative growth normalized by `n^2` must remain bounded; stable local visit counts must remain flat as unrelated graph size grows. The downstream-overlay, nested-frame, affected-component, affected-only pinning, large-pool-retention, and large-stable-collection schedules in this design are release gates, not optional tuning evidence.
...
- A planner scaling diagnostic supplements those authoritative rows with the exact size-series counters above and separate same-reference, same-membership reorder, add/replace, release-component, downstream-overlay, nested-frame, and unrelated-graph cases. It is deterministic structural evidence and does not replace the exact stable-machine benchmark comparison.
