...
Three further core pieces are required. First, a per-property **last-commit-seq**, recorded at the terminal alongside the existing per-property write timestamp, so a guard can ask "has this property committed past seq X" under `SyncRoot`. Second, a terminal **seq-guarded write** (write only if the property's last-commit-seq does not exceed a given token or equals one explicitly permitted sequence, evaluated inside the terminal's existing `SyncRoot` section), used by reconcile's guarded restore; the permitted sequence lets the restore overwrite exactly a load apply's own commit while still rejecting any newer local commit. Third, a terminal **write receipt**: every write exposes its committed sequence, or no-commit for a veto or failure, to the caller through the write context (the `IsWritten` pattern), so a load apply learns its own `applySeq` from inside the commit rather than by reading last-commit-seq afterward, which a racing local commit could corrupt. A value-based compare is deliberately rejected: a racing local write whose value coincides with the expected value would pass a value compare and then lose to the re-applied older value (see Reconcile). An outer lock around a normal setter is not an option either, because the chain's lifecycle unwind takes `_attachedSubjects`, inverting lock order.

Of #385, only the stamp half is required; its second half (in-order or FIFO delivery) is explicitly **not** a prerequisite and is not assumed; the map reconstructs order from the stamp and does not require ordered delivery. The last-commit-seq, the seq-guarded write, and the write receipt are required in addition to the stamps. Note the stamp is a *local commit* order: it totally orders local commits to a subject, but an inbound `FromSource` write is stamped when it is applied locally, so the sequence does not encode source-side causality (see Ordering limits). The stamps must land before phase 1.
...
The existing `ISubjectSource.WriteChangesAsync(ReadOnlyMemory<SubjectPropertyChange>)` contract is kept. The map constructs the outbound change from the entry: `old = sourceValue`, `new = desired`, `timestamp = timestamp`, carrying the seq, and snapshots `attemptValue = desired`, `attemptSeq = highSeq` at the moment of send. Keeping the contract means `SourceTransactionWriter` keeps its payloads, failure attribution, and rollback unchanged. Its commit path does need one coordination step, though: a transaction writes the source directly, and without ordering against the map a pending flush can write an older overlay value between the transaction's external write and its local `Confirmed` commit, after which the confirmed capture would record the transaction value as `sourceValue` while the source actually holds the flushed value. Two rules close this: the transaction holds the per-source write gate (which flush also takes per send) from before its external write **through the local apply, any rollback, and the final map settlement**, and at commit it **settles the map synchronously** under the map lock (advance `sourceValue`/`settledSeq`/`handledSeq` to the confirmed value and seq, dropping superseded overlays) instead of waiting for the asynchronous capture of the `Confirmed` change, which then gates as stale. Flush performs its liveness re-check **after** acquiring the same gate (see Flush), so a flush approved before a transaction can never send a superseded value, and no flush can interleave with a rollback. A transaction spanning multiple sources acquires all involved gates in a **deterministic order** (a stable source ordering) before its first external write and holds them until settlement completes, so concurrent transactions cannot deadlock.
...
During a load window (base-loop or in-place) source updates on owned properties do **not** advance the live registers; instead each observed value is **staged** per property. Staging is fed by the load path's applied observations directly, not by dispatched changes, so a loaded value equal to the current model (which the equality handler vetoes before dispatch) still produces a staged observation. Each staged observation carries an explicit **observation token**: the subject's commit counter read under `SyncRoot` **immediately before the apply attempt** (capturing it after an equality veto would let a local write racing the apply be misclassified as older than the observation). Ordering is defined by the token: a local commit with `seq > token` is newer than the observation; one with `seq <= token` is superseded by it, with one exception: the apply's **own** commit is the observation itself, not newer local intent. Each observation therefore also records its apply outcome, the resulting commit seq when the apply committed (`applySeq`, necessarily `> t`) or none when it was vetoed or failed, so guards can permit exactly that commit. `applySeq` is taken from the terminal **write receipt** (see Prerequisite), returned by the write itself from inside the `SyncRoot` section; it is never read back from the property's last-commit-seq after the setter returns, where a racing local commit could be mistaken for the load's commit and then wrongly permitted by the restore guard. When several observations exist for one property (overlapping loads, reordered arrival), the staging slot keeps the observation with the **highest token**; a lower-token observation never overwrites a higher-token one, regardless of arrival order.

### Flush, retry, permanent-drop

The flush loop is woken by a **composite wait** on a map-level signal set by capture, reconcile, or a backoff timer (the subscription's own wait primitive cannot be shared, since capture is its single consumer). Per send the order is fixed: **acquire the per-source write gate** (shared with transactions), then under the map lock re-check the overlay is still live (`highSeq > handledSeq`) and snapshot the attempt (`attemptValue = desired`, `attemptSeq = highSeq`), then send outside the map lock, then settle under the map lock, then release the gate. Re-checking before the gate is acquired would be a stale approval: a transaction can write and settle a newer value while flush waits on the gate, and a pre-gate snapshot would then send the superseded value. Per the confirmation mode, success settles (ack) or send-return settles (optimistic):
...
- **Overlay with `highSeq > t`:** keep the overlay untouched; flush sends it. Refresh the registers from the observation (`sourceValue = S`, watermarks advance toward `t`, `max`-guarded); the overlay invariant holds since `highSeq > t`. If the load's apply overwrote the model (`applySeq` is the property's last commit), restore `desired` through the seq-guarded write permitting exactly `applySeq`, so the model reflects the surviving local intent instead of the loaded value; the restore is a no-op when the model already holds `desired` and is rejected when an even newer local commit exists (whose capture supersedes the overlay anyway).
- **Overlay with `highSeq <= t`:** the three-way decision:
  ...
  - `S == pre-load sourceValue`, **or no pre-load `sourceValue` exists** (baseline-absent, the first-connect case): the observation gives no evidence of divergence, so the local intent is preserved. Re-apply `desired` through the terminal **seq-guarded write** (permit last-commit-seq `<= t` **or equal to the observation's `applySeq`**; a successful load apply commits above `t`, and a guard on the token alone would be tripped by the apply itself, leaving the model stuck at the loaded value while the overlay flushes), **before touching the overlay**. Three outcomes, decided under `SyncRoot`:
    ...
    - Guard fails (a local commit newer than both `t` and the apply exists): keep the overlay and the registers unchanged; the newer commit's dispatched change updates or replaces the overlay when captured. A value compare is deliberately not used: a racing local write whose value happens to equal the staged `S` would pass a value compare and be overwritten by the re-applied older `desired`, violating the token guard.
...
3. The same finally enqueues a **load-epoch marker** through the change channel regardless of outcome, carrying the load's outcome (informational; see step 4), and raises the pending-reconcile signal on the map-level signal. A marker only on success would let flushing resume after a failed load while its partially applied changes are still queued.
4. When the refcount reaches zero, the capture loop drains **through the last load's marker** (a bounded cut: everything the load applied is provably before the marker, by step 2) before flushing resumes, and reconcile runs over **whatever observations the epoch staged**, on success and failure alike. Reconcile is per-property and each staged observation is a complete, genuine source read for its property, so a partial set from a failed load is safe (unobserved properties fall to the missing-read rule) and must not be discarded: discarding a successfully observed value C while preserving a pending overlay B would let flush overwrite C with B, reopening the reconnect overwrite gap for partially failed loads. Staying suspended until a successful load is rejected too: a persistently failing load would suspend flushing forever. The retried load reconciles again; register advances are `max`-guarded, so repeated reconciles are idempotent. Draining "until observed empty" is deliberately not the rule: under sustained write load an empty queue is a moving target and reconcile could starve; the marker gives the same guarantee with a bounded wait.
...
1. **Commit-seq, attachment-generation, last-commit-seq, and the terminal seq-guarded write (prerequisite).** A per-subject commit counter stamped under `SyncRoot` and threaded via `PropertyWriteContext`; a per-subject attachment generation (volatile bump by lifecycle, read under `SyncRoot` at commit, new public core surface and a core API snapshot change); a per-property last-commit-seq recorded at the terminal (the write-timestamp pattern); a terminal seq-guarded write primitive taking a token plus one permitted exact sequence (value compares rejected: value coincidence defeats the guard); and a terminal write receipt returning the committed seq or no-commit to the caller (the sole source of `applySeq`; post-hoc last-commit-seq reads are rejected as racy). Benchmark-gated (struct growth). Must land before phase 1. The sequence is a local-commit order, not a source-causal one (see Ordering limits).
...
6. **Staged load observations with a pre-apply token.** Load observations are staged (never applied live, never re-read from the model); each carries a token read under `SyncRoot` immediately **before** the apply attempt; reconcile advances registers to the token (`max`-guarded) and treats missing observations as no-ops. Staging is fed by the load path directly, so an equality-suppressed load value still produces an observation. Each observation records the apply's resulting commit seq (`applySeq`) or its absence, so guarded restores can permit exactly the load's own commit. Per property the highest-token observation wins; overlapping loads never let a lower-token value overwrite it.
7. **Overlapping-load refcount with finally-scoped release and outcome marker.** The suspend is a generation/refcount, incremented and decremented in a finally per load, which also enqueues the epoch marker; reconcile runs over whatever observations the epoch staged regardless of outcome (per-property independence makes a partial set safe, and discarding valid observations would reopen the reconnect overwrite gap), so flushing never stays suspended and never resumes past undrained load applies or unreconciled observations. The epoch encloses the whole of `LoadInitialStateAndResumeAsync`, buffered replay included.
...
9. **Attachment-generation fence and seq-guarded re-apply.** Capture rejects a change whose stamped generation is not current (boundary races classify to the old side, by design). Reconcile re-applies run off the map lock via the terminal seq-guarded write, whose guard permits history (`<= t`) and exactly the load apply's own commit (`applySeq`) while rejecting newer local commits: the old overlay is dropped only when a fresh re-apply commit exists (recaptured as the new overlay); when the guard fails or the model already holds `desired`, overlay and registers are kept. The `highSeq > t` branch uses the same guarded restore to return the model to the surviving local intent when the load's apply overwrote it. This avoids a clobbered user write (including one whose value coincides with the staged observation), the vetoed-apply write loss, the overlay-invariant violation, and any lock-order inversion.
...
11. **Existing write contract kept; transactions coordinate at commit.** `WriteChangesAsync` still takes `SubjectPropertyChange`, and `SourceTransactionWriter` keeps payloads, failure attribution, and rollback; but the commit holds the per-source write gate from before its external write through local apply, rollback, and map settlement, and settles the map synchronously. Flush re-checks liveness only after acquiring the same gate (acquire gate, re-check and snapshot under the map lock, send, settle, release), and multi-source transactions take all involved gates in a deterministic order, so no flush can send a superseded value or interleave with a rollback, and concurrent transactions cannot deadlock.
