...
Status: draft for review (revision 10)
...
- **Overlay with `highSeq <= t`:** the three-way decision:
  ...
  - `S == pre-load sourceValue`: source unchanged (the observation adds no register information). Re-apply `desired` through the terminal **compare-and-write** (expected value: the staged `S`), **before touching the overlay**. On success the re-apply commits fresh (new seq, current generation): drop the old overlay, advance the registers to `S`/`t`, and capture turns the fresh commit into a new overlay that flush sends. On compare failure, **keep the old overlay and the registers unchanged**: the failure does not prove a newer local commit exists (the load apply itself may have been vetoed, leaving the old `desired` in the model with no later change event), so the overlay is preserved until real evidence arrives. If the model still holds `desired`, flush simply sends the kept overlay; if a newer commit caused the failure, its dispatched change updates or replaces the overlay when captured. Dropping before the compare would lose the write in the vetoed-apply case.
...
4. When the refcount reaches zero, the capture loop drains **through the last load's marker** (a bounded cut: everything the load applied is provably before the marker, by step 2) before flushing resumes. A success marker triggers reconcile from the staged observations. A failure marker instead converts that epoch's staged observations into ordinary ordered source updates (they may be partial, but each is a genuine source value and the ordered-evidence rules apply them safely), so nothing is reconciled against a partial snapshot. Draining "until observed empty" is deliberately not the rule: under sustained write load an empty queue is a moving target and reconcile could starve; the marker gives the same guarantee with a bounded wait.
...
9. **Attachment-generation fence and guarded re-apply.** Capture rejects a change whose stamped generation is not current (boundary races classify to the old side, by design). Reconcile re-applies run off the map lock via the terminal compare-and-write, compare first: the old overlay is dropped only on a successful re-apply (the fresh commit is recaptured as the new overlay); on compare failure the overlay and registers are kept until real evidence arrives. This avoids a clobbered user write, the vetoed-apply write loss, the overlay-invariant violation, and any lock-order inversion.
