...
Status: draft for review (revision 9)
...
Prerequisite: the commit-sequence and attachment-generation stamps (partial #385) plus a terminal compare-and-write; see Prerequisite
...
Correctness rests on a **commit sequence number** carried by each property change (the near-term half of #385). With it the map orders changes by commit rather than by dispatch arrival: two per-property watermarks track what has been dealt with (`handledSeq`) and what the source is confirmed to hold (`settledSeq`, with `sourceValue`), and both advance only forward. The map keeps the existing `SubjectPropertyChange` write contract, so the transaction commit path is untouched.
...
One further core primitive is required: a terminal **compare-and-write** (verify an expected current value and write, inside the terminal's existing `SyncRoot` section), used by reconcile's guarded re-apply; an outer lock around a normal setter is not an option because the chain's lifecycle unwind takes `_attachedSubjects`, inverting lock order.

Only the stamps are required. #385's second half (in-order or FIFO delivery) is explicitly **not** a prerequisite and is not assumed; the map reconstructs order from the stamp and does not require ordered delivery. Note the stamp is a *local commit* order: it totally orders local commits to a subject, but an inbound `FromSource` write is stamped when it is applied locally, so the sequence does not encode source-side causality (see Ordering limits). The stamps must land before phase 1.
...
Goals: close 1 to 5 above for sources (phase 1) and the unbounded-server-memory case (#281, phase 2); bound outbound memory structurally; give one reconcile policy that every load path calls; keep the writer hot path lock-free.
...
Baseline-absent (no `sourceValue` yet) defaults to send-current.
...
- **No overlay, or overlay with `highSeq <= t`:**
  - `S == desired`: the source already holds the intended value; registers to `S`/`t`, **drop** the overlay (nothing to send).
  - `S == pre-load sourceValue`: source unchanged. **Drop** the old overlay, set the registers from the observation, then re-apply `desired` through the terminal **compare-and-write** (expected value: the staged `S`). If the model still holds `S`, the re-apply commits fresh (new seq, current generation) and capture turns it into a fresh overlay that flush sends. If the compare fails, a concurrent local write committed after the token and its own dispatched change creates the fresh overlay when captured, so nothing is lost either way.
  - otherwise: the source diverged; drop the overlay (source wins), registers to `S`/`t`.
...
1. Each load increments a **suspend generation/refcount** (a plain boolean is unsafe: the first reconcile would clear it while a second load is still applying) that suspends flushing, live-register advance from source updates, and overlay drop for owned properties, and enables staging. The increment and its matching decrement are scoped in a **finally**, so a failed or cancelled load always releases the suspension and wakes the loop; a failed load's staged observations are **discarded** (they may be partial), and its already-enqueued applies are later captured as ordinary source updates, which is safe under the ordered-evidence rules.
2. The load runs. Its source applies dispatch synchronously during each apply's unwind, so by program order (happens-before, even if the physical thread changes across the load await) they are all in the subscription queue before the next step.
3. On success the load enqueues a **load-epoch marker** through the same channel and raises the pending-reconcile signal on the map-level signal.
4. When the refcount reaches zero, the capture loop drains **up to the last load's marker** (a bounded cut: everything the load applied is provably before the marker, by step 2), then reconcile runs from the staged observations, then flushing resumes. Draining "until observed empty" is deliberately not the rule: under sustained write load an empty queue is a moving target and reconcile could starve; the marker gives the same guarantee with a bounded wait.
...
1. **Commit-seq and attachment-generation stamps, plus terminal compare-and-write (prerequisite).** A per-subject commit counter stamped under `SyncRoot` and threaded via `PropertyWriteContext`; a per-subject attachment generation (volatile bump by lifecycle, read under `SyncRoot` at commit, new public core surface and a core API snapshot change); and a terminal compare-and-write primitive. Benchmark-gated (struct growth). Must land before phase 1. The sequence is a local-commit order, not a source-causal one (see Ordering limits).
...
6. **Staged load observations with a pre-apply token.** Load observations are staged (never applied live, never re-read from the model); each carries a token read under `SyncRoot` immediately **before** the apply attempt; reconcile advances registers to the token (`max`-guarded) and treats missing observations as no-ops. Staging is fed by the load path directly, so an equality-suppressed load value still produces an observation.
7. **Overlapping-load refcount with finally-scoped release.** The suspend is a generation/refcount, incremented and decremented in a finally per load; a failed load discards its staged observations and wakes the loop, so flushing can never stay suspended.
8. **New primitives.** A map-level wake signal for the composite wait, and a load-epoch marker enqueued through the change channel at load completion (the drain cut; "drain to empty" is rejected as starvation-prone). The subscription's own wait primitive is untouched.
9. **Attachment-generation fence and guarded re-apply.** Capture rejects a change whose stamped generation is not current (boundary races classify to the old side, by design). Reconcile re-applies run off the map lock via the terminal compare-and-write (drop the old overlay, recapture the fresh commit), avoiding a clobbered user write, the overlay-invariant violation, and any lock-order inversion.
...
11. **Existing write contract kept.** `WriteChangesAsync` still takes `SubjectPropertyChange`; the transaction commit path is untouched.
