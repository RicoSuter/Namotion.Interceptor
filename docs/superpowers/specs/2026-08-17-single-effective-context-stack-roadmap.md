...
The experimental #419, #472, and #440 branches remain sources of tests and reproduced schedules. Their production implementations are not transplanted wholesale. PR #474 is behavior-neutral infrastructure. PR #419 is the coordinated semantic cutover. PR #472 generalizes authority identity. PR #440 builds hosted lifecycle state on the resulting ownership and authority model.
...
Resolution order remains local services, public fallbacks, then the one internal ownership route. Ordering attributes apply to the complete gathered service set. Repeated paths to the same service instance deduplicate by the existing service contract. Nonunique services remain intentionally plural.
...
`AddProperties` remains supported at any time and participates in the topology turn. One public call is one atomic, once-materialized metadata batch. Nonstructural metadata publishes without lifecycle work. Direct-readable structural metadata added to a lifecycle-owned subject is handled as one synthetic structural transition so metadata, committed membership, baseline, route, and callback projections agree. Registry binds an exact pending `RegisteredSubjectProperty` to an exact one-shot Core commit notification. Core claims that identity as the first `AddProperties` entry action before enumeration or application code, keeps it publicly invisible before commit, and promotes that exact wrapper at Core commit before later user callbacks.
...
- Preserve later committed nested generations. Stale callback tails stop through exact generation and route-descriptor checks.
...
- Preserve position-dependent repeated-continuation behavior: repetition upstream of the coordinator invokes and reconciles it per call; repetition downstream performs ordered stores inside one coordinator invocation and reconciles once to the final successfully stored value, including before rethrow after a later failure.
...
- Make the exact single `ILifecycleInterceptor` resolved by an ownership domain its canonical recursive-ownership coordinator. `WithLifecycle()` installs the standard built-in provider; an advanced registration of one other exact implementation is equally canonical. Zero instances mean explicit-root-only operation and two distinct instances are incompatible. Remove optional inheritance configuration and functional `ContextInheritanceHandler` behavior.
- Keep the normally visible public provider facade capability-minimal, stack-only, generic where write values require it, and reservation-oriented. Core alone gates, commits, restarts, cancels, and finalizes. Tracking owns traversal and reconciliation. Advanced extension implementations are supported under strict no-retain, synchronous, concurrent-call, and no-throw obligations.
