...
Follow the repository's PR conventions (AGENTS.md, "Pull Requests" section, which may be newer than this worktree's copy: re-read it at PR time). Specifically: prefix the title with a conventional-commit type, so `feat: Stable-ID subject update protocol and ID-resolving pipeline`; fill in `.github/pull_request_template.md` by opening the file and completing its sections rather than bypassing it with a bare `--body-file`; and apply one `area:` and one `type:` label at creation via `gh pr create --label`, choosing from `gh label list` (expected: `area: connectors` and `type: feature` or the closest equivalents that actually exist).

The description must include: the approved break list PLUS the two additions discovered in planning (`Count` removed, `CompleteSubjectIds` added); the release-notes paragraph for the AspNetCore JSON shape change (structural items serialize as `id`/`key` instead of `index`; `root` may be absent on partial updates); what was deliberately not ported and why (one line each: pending buffer, lazy minting, apply lock, digest); the read-only and immutable collection support restored after the port regressed it (the reference branch had dropped the capability and deleted its tests, including a silent-data-loss path where a populated read-only dictionary serialized as `Items=null`); the one unrelated pre-existing flake fixed here (`ConnectorMetricsTests` thread-pool starvation, same family as master's #477, called out so a reviewer can ask for it to be split); a "pending long-running verification" section listing the benchmark and the two Connector Tester runs as open gates the user runs on a separate machine. Do NOT mention closing #197 here; per the spec it closes only once all three stack PRs are implemented and reviewed. No AI attribution. Spec reference: `docs/superpowers/specs/2026-08-18-websocket-structural-stack-design.md`.
