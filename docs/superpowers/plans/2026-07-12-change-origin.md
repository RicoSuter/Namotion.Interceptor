...
Run: `dotnet run --project src/Namotion.Interceptor.Benchmarks -c Release -- --filter "*Write*"` (check the project's README or `--list flat` for the exact write benchmark names). Compare against master numbers (run the same filter on a master checkout if no baseline exists). Regressions above noise on the plain local write path are a stop-and-analyze signal.
