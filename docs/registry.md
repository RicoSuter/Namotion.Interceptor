...
Dynamic properties participate in lifecycle tracking when `WithLifecycle()` or `WithFullPropertyTracking()` is enabled. A stored dynamic property holding a reference to another subject attaches that subject and counts it. Derived properties are excluded: a dynamic property computes its value through consumer delegates over state the graph cannot see, so `[Derived]` on one declares a cache rather than the store of record. So an `AddDerivedProperty<Tire>("FirstTire", ...)` that returns the first tire from a collection leaves that tire at reference count 1, from the collection property alone. A subject reachable only through a derived property is never tracked, and the derived change handler rejects it rather than letting it go silently unowned.
