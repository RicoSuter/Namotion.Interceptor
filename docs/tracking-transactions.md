...
Stage 2 applies source-bound changes marked with a `Confirmed` origin carrying the source that accepted them in stage 1, so the outbound change queue treats their notifications as echoes and a committed value is written to its source exactly once. See [Change notification source semantics](connectors.md#change-notification-source-semantics) for the full contract, including cascade and derived property behavior. Value-transforming write interceptors remain discouraged with source transactions: stage 1 writes the captured value to the source while stage 2's commit replay runs the interceptor and stores the transformed one, so the two momentarily disagree. That disagreement self-heals rather than persisting, because the survival check sees the replayed value differ from the confirmed value and demotes the change to `Local`, so it is not echo-suppressed and the transformed value flows back to the source on the next outbound flush. Prefer transforming at capture (an `OnChanging` hook), so the transaction stages the final value directly.
