...
- **Shared subjects** have no identity across updates, so a subject referenced from two places arrives on the receiver as two instances unless both references are in the same update. The C# applier also applies a payload only at the first reference to its ID within an update: a second reference to the same ID, whose property is still empty on the receiver, gets a new subject without properties.
