...
**Subjects without Registry metadata:** a structural value can reference a subject that has no Registry metadata: one whose attach is still in progress while the update is built, one that left the graph after its change was captured, or one written into a subject-holding property in place so that it was never attached. The update states such a subject complete from its own property metadata, counted in `SubjectUpdateDiagnostics.MetadataFallbackSerializations`, so that the members listed next to it still arrive. Processors filter on Registry metadata, so they cannot filter the properties of such a subject, and it carries no attributes: do not rely on processors as a confidentiality boundary. A property change whose own subject has no Registry metadata any more is dropped and counted in `SubjectUpdateDiagnostics.DroppedOutboundChanges`; the change that attaches the subject again carries its complete state.
...
- A subject held at the position takes the payload of the ID the update names there and adopts that ID, which is how the children a receiver constructs itself are matched to the sender's on the first complete update; later updates then address them by ID. A held subject that already has another ID gives it up for the named one, as after a sender restart with new IDs or when the receiver assigned IDs of its own first, unless this apply already names the held subject by its current ID. For a collection the held subjects are matched by position, for a dictionary by key.
- Structure the receiver cannot store is dropped: an ID that already names another subject of the receiver, a subject where none is held, or a membership that differs from the held one. Nothing is created for it. One warning per update names every such property once, qualified by the type of the subject owning it. A dictionary entry without a key is counted in `DroppedInboundSubjectUpdates`.
