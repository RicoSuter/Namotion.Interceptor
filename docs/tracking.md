...
Passing an `IScheduler` moves delivery off the writing thread: the write enqueues the change and returns, and a drain on the scheduler delivers it afterwards. The `Schedule` call itself still happens inside the write, so this holds only for a scheduler that does not block there. A scheduler whose `Schedule` blocks blocks the writer, and the scheduler must not do that for the same reasons `onError` must not. Four overloads take one, `property.Subscribe(observer, scheduler, onError)` and `property.Subscribe(callback, scheduler, onError)` plus the two matching `subject.SubscribeToProperty(x => x.Foo, ..., scheduler, onError)` forms, where `onError` is optional. They return a `ScheduledPropertySubscription` instead of a bare `IDisposable`, and disposing it is mandatory for the same reasons as above.
...
**The observer still must not block**: the drain owns the scheduler's thread for the whole call, so a blocking observer starves every other subscription on that scheduler. Two subscriptions on one `EventLoopScheduler` with one observer blocking leaves the other delivering nothing at all, with a growing `PendingCount` and no error reported anywhere. The `MaxBatch` handoff shares a thread fairly only between observers that return.

**Isolated from the writer**: an exception from the observer, from `onError`, or from the scheduler can never propagate into the write or suppress another channel's delivery for that write. An observer exception is reported to `onError` and the subscription stays live. A scheduler exception is reported too but faults the subscription, which stops accepting changes for good and is what `ScheduledPropertySubscription.IsFaulted` reports. The isolation is one-way, and the converse loses the change entirely: this channel sits downstream of every other consumer, so a throwing lifecycle handler or a throwing inline observer earlier in the same write unwinds it before the change is enqueued. The value is still committed to the model, the scheduled observer never hears about it, `onError` stays silent and `PendingCount` reads zero.

**Ordering and staleness**: deliveries arrive in the order the interceptor pushed them, which under concurrent writes to the same property is not commit order, exactly as for inline delivery. Deferred delivery widens the staleness window from microseconds to however long the queue is, so read `change.GetCurrentValue<TValue>()` rather than the delivered new value when you need the current one. That read can throw where the inline one practically cannot: for an `object`-typed or otherwise polymorphic property, a window that wide lets the runtime type move on, and `GetCurrentValue<TValue>()` then throws `InvalidCastException` instead of returning something merely stale, which without an `onError` handler is a silently skipped delivery. Staleness is also not only a question of which commit a value describes. A delivered reference-typed value is the live object rather than a snapshot, so the writer can keep mutating it after the delivery: deferral corrupts nothing by itself, but an array or other mutable object handed to the observer is shared with whoever wrote it.
...
**Ambient context does not flow to the observer**: the drain is scheduled with `ExecutionContext` flow suppressed, so the writer's `AsyncLocal` values, `Activity.Current`, and logger scopes do not reach it. That is deliberate, because one drain batch delivers changes from many writers and a flowed context would stamp whichever writer enqueued first onto all of them, which is worse than no context at all. Suppression governs the context a work item carries, not the context a scheduler's worker thread was born with, so prefer `Scheduler.Default`: the thread pool applies the execution context per work item, which makes the suppression the whole story. A scheduler that owns a thread, such as `EventLoopScheduler`, creates that thread on its first `Schedule` and the thread then keeps whatever `AsyncLocal` values were ambient at that moment for life, which nothing here can strip. A caller-owned scheduler like that must therefore be created outside any transaction scope. A worker thread born inside one carries `SubjectTransaction.CurrentTransaction` for the rest of its life, so every property write an observer makes on it lands in a transaction that was disposed long ago and whose pending-changes buffer has since returned to the static pool and been rented by a live transaction. The write then vanishes from its own call path and is committed by that unrelated transaction, with nothing thrown and `onError` never firing.
...
**Notifications are not serialized, so the sequence violates the Rx grammar.** `OnNext` is raised straight from the writing thread, and concurrent writers to one property raise it concurrently on one observer. The context-level observable is wrapped in `Subject.Synchronize()` precisely to be grammar-conformant; this one deliberately is not. Rx operators assume serialized notifications and most stateful sinks keep their state unguarded, so apply `.Synchronize()` before any stateful operator, including `Take`, `Skip`, `Scan`, `DistinctUntilChanged` and `Buffer` by count. Skipping it corrupts rarely rather than never, which is the worse failure mode: an unsynchronized `Take(1)` fed by four writer threads forwarded more than one item in 1 round out of 2000.

**Adding any operator changes what a throwing handler does.** Operators wrap the observer in the auto-detaching decorator the bare subscribe deliberately avoids, so instead of propagating to the writer and leaving the subscription live, the first handler exception tears the subscription down silently and the stream simply stops. A handler composed over this observable must not throw at all.

**Disposing what `Subscribe` returns is mandatory**, exactly as for `SubscribeInline`. The sequence never completes, so nothing disposes the subscription for you, and Rx users routinely drop handles for streams they treat as infinite. A dropped handle keeps the observer receiving changes and permanently disables the process-wide idle write fast path, with no finalizer and no recovery for the process lifetime.

Two further hazards when composing the off-thread hop by hand, which is why the scheduler overloads exist. `ObserveOn` dedicates a private thread to each subscription when `Scheduler.AsLongRunning()` resolves an `ISchedulerLongRunning`, which it does for both `Scheduler.Default` and `TaskPoolScheduler` through their `IServiceProvider` implementation rather than a direct interface cast, so composing it per property is unaffordable. That thread is taken on the first signal rather than at subscribe, so an idle property hides the cost until it stops being idle. And an exception from the handler escapes the `ObserveOn` sink into the scheduler, which does not catch it: on `Scheduler.Default` it goes unhandled and terminates the process, and on `TaskPoolScheduler` it becomes an unobserved task exception and the subscription stops delivering, so one bad change silently ends the stream. The scheduler overloads have neither.
...
- **Per-property observers are not serialized**: an `IPropertyChangeObserver` or `PropertyChangeCallback` may be invoked concurrently on multiple threads and must be thread-safe, fast, non-blocking, and must not throw. Wrap failing work in a try-catch internally. (The context-level observable is serialized through `Subject.Synchronize()`; the per-property `GetInlineChangeObservable()` is not; the queue is single-consumer per subscription.) The [scheduled overloads](#scheduled-delivery) serialize per subscription, so an observer used by exactly one of them is never re-entered and needs no synchronization; one shared across several subscriptions is still invoked concurrently.
