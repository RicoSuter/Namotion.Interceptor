...
- **Ordering**: under concurrent writes to the same property, notifications may arrive out of commit order. If you need the current value, re-read the property rather than relying on the delivered new value: `change.GetCurrentValue<TValue>()` does this for you, reading the property now instead of returning the value captured when the change was created, without needing to keep a separately typed reference to the subject. `GetOldValue<TValue>()` is the value the property held immediately before the write committed, read under the subject lock, including when the subscription raced the write. For one property, changes ordered by `Revision` therefore chain: each old value is the preceding change's new value, whichever order they were delivered in.
...
Both values are exact on every channel: the terminal write reads the old value under the subject lock immediately before it stores the new one, so a change describes the transition it committed even under concurrent writers. A collapse keeps the oldest old value and the newest new value of the changes it merged, which is the transition across the whole batch. The exceptions are a derived property's recalculation, whose old value is the value the last recalculation settled on rather than a locked read, a write whose caller supplies the old value itself, such as `SetPropertyValueWithInterception`, and a subject hierarchy whose base assembly was built by generator version 0.9.3 or earlier, where the setters read the old value before the write starts (see [Writing a base class by hand](generator.md#writing-a-base-class-by-hand)). Compare `Revision` or re-read the property if you need to know which change is the latest.
