...
- **Per-property observers are not serialized**: an `IPropertyChangeObserver` or `PropertyChangeCallback` may be invoked concurrently on multiple threads and must be thread-safe, fast, non-blocking, and must not throw. Wrap failing work in a try-catch internally. (The context-level observable is serialized through `Subject.Synchronize()`; the per-property `GetInlineChangeObservable()` is not; the queue is single-consumer per subscription.) The [scheduled overloads](#scheduled-delivery) serialize per subscription instead.
