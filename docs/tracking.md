...
(b) Per property, a flush collapses to the newest commit in that batch. Collapsing is within a flush only: a commit that a previously delivered one has already superseded is still emitted, so a consumer that needs the current value re-reads the property rather than assuming arrival order matches commit order.
