...
- **Ordering**: under concurrent writes to the same property, notifications may arrive out of commit order. If you need the current value, re-read the property rather than relying on the delivered new value: `change.GetCurrentValue<TValue>()` does this for you, reading the property now instead of returning the value captured when the change was created, without needing to keep a separately typed reference to the subject. `GetOldValue<TValue>()` is the value the property held immediately before the write committed, read under the subject lock, so for one property the changes chain when ordered by `Revision` even though they may arrive in another order. [Delivery Guarantees](#delivery-guarantees) lists the writes whose old value is instead the value the caller passed.
...
Both values are exact on every channel for a generated partial property: the terminal write reads the old value under the subject lock immediately before it stores the new one, so a change describes the transition it committed even under concurrent writers, and for one property the changes ordered by `Revision` chain, each old value being the preceding change's new value. Revisions still decide *which* change survives a collapse, not that its old value is the value the property held at the preceding surviving revision. Three kinds of write carry the old value the caller passed instead, which was read outside the lock and can already be superseded by the time the write commits: a property whose metadata has no stored-value reader, namely a registry or dynamic property (`AddProperty`, `DynamicSubjectFactory`), a hand-written property registered without [`WithStoredValueReader`](generator.md#writing-a-subclass-by-hand) and a partial override that omits its setter; a property declared on a base assembly built by generator version 0.9.3 or earlier, whose metadata predates the reader; and a write whose caller supplies the old value, namely a derived recalculation, which reports the value the previous recalculation settled on, and `SetPropertyValueWithInterception` on a value-type property. Compare `Revision` or re-read the property if you need to know which change is the latest.
