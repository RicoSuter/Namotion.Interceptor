...
The drain always runs and cannot be switched off, so a stop can block on an unreachable endpoint. It is bounded internally at 5 seconds per connector. On a hosted connector the host's shared `HostOptions.ShutdownTimeout`, 30 seconds by default, bounds the total, and that is the knob to reach for when a host runs several connectors that can hang. A connector torn down while the host keeps running, such as a source on a detached subject, has only the internal bound.

For a source the batch goes through the normal write handler, which flushes the retry queue first because that backlog holds older commits and must keep its place in commit order. A deep backlog can consume the bound on its own and park the batch, so the source flushes once more under a bound of its own while the transport is still up. A stop therefore costs up to twice the bound in the worst case, around 10 seconds.
