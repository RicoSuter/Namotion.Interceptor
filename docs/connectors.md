...
- Writes the source refuses until it reconnects are held back rather than re-sent: see [Writes Refused Until Reconnect](#writes-refused-until-reconnect)
...
The cost is that a stop can block on an unreachable endpoint, so the wait is bounded at 5 seconds per connector. The bound is enforced from outside the write as well as through its cancellation token, because a handler is free to ignore the token and one does: the OPC UA server writes synchronously under the SDK's node manager lock. A write abandoned at the bound is not cancelled by the abandonment, but nothing waits for it either: it may already be in flight, or it may not have started yet and then run under a token that has reached the same deadline, in which case the write fails and the batch is lost. The stop does not learn which happened. A late write that does reach the wire fails cleanly rather than reviving a connection, because the clients here reject a write once the connection is disposed.

The bound is fixed rather than configurable. Connectors stop one after another under the host's shared `HostOptions.ShutdownTimeout`, 30 seconds by default, which is the budget a consumer actually tunes.
...
### Writes Refused Until Reconnect

A source can name a subset of its failed changes as refused for the lifetime of its connection, through `WriteResult.WithRefusedUntilReconnect`. The retry queue keeps those changes but stops sending them until the connection is replaced, and a flush whose only failures are refusals reports success.
...
- The held set sits **outside** `writeRetryQueueSize` rather than inside it, because dropping a held write loses one the next connection would have taken. What bounds it is one entry per distinct property refused on the current connection, which is not the model's current property count: nothing evicts an entry when its subject detaches, so a model that attaches and detaches subjects whose writes are refused accumulates one entry per churned property, each keeping its detached subject graph reachable until the next connection replacement releases the set. It follows the refusal churn rather than the write rate and can exceed the configured queue size, which is why `Diagnostics.HeldWrites` reports a `null` capacity. Once released they are ordinary pending writes and the ring buffer applies to them from there.
...
**WriteResult**: Return `WriteResult.Success` when all changes were written. Return `WriteResult.Failure(changes, exception)` when changes failed, whether or not others in the batch succeeded. The failed changes list everything not confirmed written; unlisted changes count as written. Enumerate the changes the source refused, and use `WriteResult.CallFailed(exception)` only when the call itself failed (a timeout, a dropped connection, a faulted session): that reports the whole attempted batch failed and additionally stops the flush. A set of changes larger than `WriteBatchSize` is split into batches, and a batch that names the changes it refused does not hold back the ones behind it; their failures are reported together in a single result. A batch that fails without naming one stops there, so the batches behind it are not each handed to a source that is not answering, and they are reported unconfirmed. The base class enqueues the failed changes into the write retry queue automatically. If the source will keep refusing some of them until it reconnects, name that subset with `WithRefusedUntilReconnect` so the queue stops re-sending them, see [Writes Refused Until Reconnect](#writes-refused-until-reconnect).
...
- `HeldWrites` above zero means the source refuses writes to those properties and is waiting for its next connection to deliver them. Unlike the other three it is a held set rather than a queue and sits outside any configured bound, which is why its `Capacity` is `null`; what bounds it is described in [Writes refused until reconnect](#writes-refused-until-reconnect).
