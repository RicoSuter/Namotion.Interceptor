...
The `Origin` of a change notification is typed (`ChangeOrigin`): `FromSource` when an inbound update stored exactly the value the source sent, `Confirmed` when a source transaction commit stored the value the source acknowledged, `Correction` for a synthesized re-assertion of the stored value to a source that diverged from it (see below), and `Local` for everything else. A change carries a source when its stored value is exactly the value that source sent or confirmed, or, for a `Correction`, to record which source diverged. The outbound change queue skips changes whose origin source is the target source itself (corrections excepted, see below), so a committed value is written to its source exactly once, by the commit.
...
When an inbound value is projected by a hook to the value the model already stores (the model at 100 receiving 105 with a clamp to 100), the equality check suppresses the write and no ordinary change exists. The library then publishes a `Correction` change (old and new value both the stored value) so the diverged source converges to the model. A correction carries a fresh assertion timestamp, stamped at synthesis and re-stamped on every delivery attempt: endpoints may order inbound writes by timestamp (the OPC UA read-after-write check uses exactly this pattern), so a correction must never be rejectable as stale, and corrections are never retried. The stamp is bounded below, Lamport style, by the triggering inbound timestamp, the property's write-timestamp, and the processor's previous correction attempt (a processor-wide ratchet, so monotonicity holds across separate corrections too): when the local clock lags that bound (source clock skew, frozen test clocks), the stamp advances one tick past the bound instead, so an ordering endpoint always sees a strictly newer stamp. A bound already at `DateTimeOffset.MaxValue` has no representable successor; such a correction is dropped with a warning rather than thrown on or written at the ceiling. An endpoint whose rejection bar is unobservable (one that orders by an internal clock ahead of both our clock and every timestamp it ever sent) cannot be beaten by any client-side stamp; that residue is reconciliation territory (`RequestResynchronization`, #342). The property's write-timestamp metadata is not advanced (an unchanged value is not a new write), so the model keeps the value's real last-change time, and model and source may intentionally disagree on the timestamp of an identical value. The source's echo of the corrected value is suppressed by the value-based equality check. No backing field is written and no hooks or `PropertyChanged` fire.

Corrections bypass the own-source echo skip and flow through every change queue processor whose property filter matches, so delivery is decided by topology rather than by the diverged source's identity: single-owner sources converge to owner-only delivery through their property filters, WebSocket broadcasts the authoritative projection to all replicas, and re-writing an unchanged value to another bound source is idempotent. Corrections are delivered in both buffered and immediate modes: send-time revalidation, not flush dedup, is what guards against a stale correction, and a processor without a buffer runs each correction through the same revalidation path. In a buffered flush, normal changes are preserved as one batch and written before corrections, which are then delivered individually; cross-property queue order is not preserved across this partition.

Each correction is revalidated at send time: the processor re-reads the current model value immediately before the write and drops the correction if the model already moved, and after the write it re-reads again and, when an inbound value from the same source landed in flight, writes a follow-up correction in a bounded loop, dropping the correction with a warning that names `RequestResynchronization` and #342 once the loop is exhausted.

Corrections never reach the change observable, so application-level subscribers see nothing; direct change queue subscribers do see them and should filter on `Origin.Kind` when only model mutations are wanted. The synthesis-time write-timestamp check is advisory because timestamps can alias, so a queued correction is a re-assertion signal rather than a guaranteed-current value command. A direct subscriber that acts on its value must revalidate it against the live model at send time, as `ChangeQueueProcessor` does. `Confirmed` transaction writes never produce corrections. A correction is produced only when the source actively sends a diverging value; reasserting the model to a silently diverged source is a reconciliation concern (#342).
