...
That memory is kept for as long as a property is part of the object graph and dropped when its subject detaches, so the guarantee has no time or volume window: a live property keeps its baseline however long it stays quiet. Detached subjects are released rather than retained, which matters because the memory is keyed by property reference and a property reference holds its subject.
