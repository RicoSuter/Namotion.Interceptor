...
- Automatic retry when `WriteChangesAsync` fails during normal operation: ahead of the next outbound batch, and by an idle flush every `retryTime` while the source is `Synchronized` and each time it becomes `Synchronized`
