...
When an inbound value is projected by a hook to the value the model already stores (the model at 100 receiving 105 with a clamp to 100), the equality check suppresses the write and no ordinary change exists. The library then publishes a `Correction` change (old and new value both the stored value) so the diverged source converges to the model. A correction re-asserts the value the model already holds, so it carries the property's existing write-timestamp (the value's real last-change time) and does not advance the write-timestamp metadata (an unchanged value is not a new write). The source's echo of the corrected value is suppressed by the value-based equality check, so no fresh timestamp is needed. No backing field is written and no hooks or `PropertyChanged` fire.
