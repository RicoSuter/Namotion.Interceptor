...
When an inbound value is projected by a hook to the value the model already stores (the model at 100 receiving 105 with a clamp to 100), the equality check suppresses the write and no ordinary change exists, yet the source still holds its diverging value. The library then publishes a `Correction` change (old and new value both the stored value) so the diverged source converges to the model. A correction re-asserts an existing value: it writes no backing field and fires no hooks or `PropertyChanged`, and it never reaches the change observable, so application-level subscribers see nothing. Direct change queue subscribers do see corrections and should filter on `Origin.Kind` when only model mutations are wanted. A correction is produced only when a source actively sends a diverging value; a source that silently diverges (changes its value without notifying) is a reconciliation concern, not a correction one.

Corrections bypass the own-source echo skip and flow to every change queue processor whose property filter matches, so delivery follows topology rather than the diverged source's identity, and they are delivered in both buffered and immediate modes. Each correction is revalidated against the live model at send time: it is dropped if the model already moved, followed up if an inbound value from the same source lands during the write, and after a bounded number of unconverged attempts dropped with a warning naming `RequestResynchronization`. A dropped correction leaves the source diverged only until its next inbound event. Corrections are safe under transactions: `Confirmed` transaction writes never produce them, a correction only ever asserts committed state, and a pending (uncommitted) value is never written to a source.

`Correction.ChangedTimestamp` records when the correction assertion was created or sent. It does not guarantee monotonic wire ordering, and a connector that requires monotonic versions must provide them in its own outbound writer. A correction does not advance the property's write-timestamp metadata (an unchanged value is not a new write), so the model keeps the value's real last-change time and model and source may intentionally disagree on the timestamp of an identical value.

Values passed to `SetValueFromSource` must be of the property's CLR type; an enum's underlying integral type is also accepted (the OPC UA wire shape), so enum values that arrive as boxed integers compare and store correctly.

Two limitations are inherited from the local-first sync model rather than introduced by corrections. First, outbound changes carry the value's semantic change time, not a monotone wire clock, so an endpoint that orders writes by timestamp can reject a write whose model time lags its own clock; that is endpoint-specific and handled in the connector's outbound writer. Second, because inbound applies are not ordered against outbound writes, a notification a source emitted before a write but delivered after it can revert the model to the older value, which is then echo-suppressed and does not flow back (this affects any outbound write, not just corrections). Both recover on the source's next inbound event, through read-after-write where the connector supports it (OPC UA, on by default), or through explicit resynchronization.
