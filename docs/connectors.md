...
**What keeps the two ends in sync** is not the conflict rule but the delivery path: the newest local commit is never dropped, so the source always receives the model's settled value, and the source's notifications carry its own value back, so the model converges to what the source holds. A source that cannot meet the read-back rule on [`ISubjectSource.WriteChangesAsync`](../src/Namotion.Interceptor.Connectors/ISubjectSource.cs) is outside that guarantee.
...
- Automatic retry when `WriteChangesAsync` fails during normal operation: ahead of the next outbound batch, and by an idle flush every `retryTime` while the source is `Synchronized` and each time it becomes `Synchronized`, except while a connector's own reconnect has yet to reconcile the parked writes
...
The cost is that a stop can block on an unreachable endpoint. Final delivery and the source retry handoff share one internal five-second safety bound, which cannot be configured per connector. An idle flush stuck in a write that ignores cancellation adds a second bound of the same length. Connectors stop one after another under the host's shared `HostOptions.ShutdownTimeout` of 30 seconds by default, so the host timeout must leave enough time for every connector to complete or reach its internal bound.
...
RunAsync
 ...
 └── retry loop (per connection attempt)
      ...
      └── ProcessAsync()                   ← drains changes, calls your WriteChangesAsync; an idle flush retries parked writes alongside
```
...
**Constructor parameters**: `bufferTime` (default 8ms) controls the change queue batching window. Changes within this window are coalesced into a single `WriteChangesAsync` call. `retryTime` (default 10s) controls the delay between retry attempts when `StartListeningAsync` or the pump loop fails, and the interval of the idle flush that retries parked writes while the source is `Synchronized` (see [Write Retry Queue](#write-retry-queue)). Outside 1 ms to about 24 days the interval is off and only the flush on becoming `Synchronized` runs.
...
**Read accepted writes back**: the model must receive the peer's value for each property the peer accepts, without a further local write. See [`ISubjectSource.WriteChangesAsync`](../src/Namotion.Interceptor.Connectors/ISubjectSource.cs) for the rule and what satisfies it.
...
**A write the retry queue drops, or the peer keeps rejecting, leaves the two ends diverged.** The [write retry queue](#write-retry-queue) retries a failed write, but it is a bounded ring buffer that drops its oldest entries when full, and with it disabled the change is logged and dropped immediately. A write the peer rejects every time stays queued and is retried, with a warning, every `retryTime`. Tracked as [#342](https://github.com/RicoSuter/Namotion.Interceptor/issues/342).
...
**OPC UA and MQTT internal reconnects skip the reconcile.** Transport-level reconnects handled inside those connectors (the OPC UA health loop and the MQTT monitor) reload initial state without running the connect-window reconciliation, so parked writes are replayed as they are and the model shows the source's older value after `Synchronized` until the replayed write is read back under the rule on [`ISubjectSource.WriteChangesAsync`](../src/Namotion.Interceptor.Connectors/ISubjectSource.cs). OPC UA reads nothing back for a node that cannot be monitored while `EnablePollingFallback` is disabled, or whose deadband or data change trigger suppresses the replayed value, so there the model keeps the older value until the next load. The WebSocket client does reconcile after its own monitor reconnect: from the moment it detects the connection loss it parks outbound writes, and once the reconnect has loaded the server's state it judges them the same way as the connect-window reconciliation. Tracked as [#362](https://github.com/RicoSuter/Namotion.Interceptor/issues/362).
...
**A source that neither answers reads nor echoes writes is unobservable.** If it clamps or rejects a value internally and sends no notification, nothing local can reveal the difference and the two ends stay diverged. Convergence rests on the read-back rule on [`ISubjectSource.WriteChangesAsync`](../src/Namotion.Interceptor.Connectors/ISubjectSource.cs). See [#373](https://github.com/RicoSuter/Namotion.Interceptor/issues/373).
