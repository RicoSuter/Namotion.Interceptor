...
Inbound source values are never validated: the write interceptor runs validators only for locally originated writes, so a validator cannot reject a value a source sent or confirmed. A derived property that recalculates because of an inbound write is a local write and is still validated. See [Validation](validation.md#validation-is-scoped-to-local-writes).
