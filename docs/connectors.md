...
Suppression is not limited to the buffered path. A source with `bufferTime` at zero has no batch to collapse, but each change is still checked against what has already been delivered before it is written.
...
That memory is held on the subject itself rather than by the connector, so it is collected with the subject and the guarantee has no time or volume window: a property keeps its baseline for as long as it exists, however long it stays quiet. It is kept per source, so a model exposed over several protocols gives each connector its own baseline and one connector's delivery cannot suppress another's.

What the memory cannot do is retract a decision already made. Admission happens when a flush is assembled, so an inbound value that arrives after that point but before the batch reaches the source does not suppress anything in it, and a local commit the source has already superseded can still be written out. The window is the duration of one write rather than unbounded, and closing it would mean holding the batch until the write completed, which is a worse trade than the case it covers.

[Source transactions](tracking-transactions.md) write to the source themselves and then apply locally, and that local apply arrives here as a confirmation. Normally it is not sent on, because the source already has it. The exception is when this processor has also written that property itself: such a write can reach the source after the transaction's and leave it holding an older commit than the subject, so the confirmation is sent out to restore it.

That "has also written it" mark is sticky for the life of the connection, and deliberately so. Nothing observable on this side can prove that an earlier write of ours did not land on the source after a transaction's direct write: an inbound echo cannot be matched to the write it acknowledges, and a write of a value the source already holds usually produces no echo at all. Clearing the mark on any such event would therefore be a bet against an ordering the client cannot see, and losing it silently strands a committed transaction value. A property written only through transactions never sets the mark and costs nothing extra, while one written both ways pays a redundant write per transaction to stay converged.
...
A delivered revision is recorded before the write is attempted, so after a failure the recorded baseline is ahead of what the source actually holds. A newer commit has a higher revision and is still admitted, so the next write to that property repairs the divergence. An older straggler is suppressed, which leaves the source where it is rather than moving it to a value the subject has already superseded.
...
### A write racing the initial-state load has a timing-dependent winner

Whether the local write or the source's snapshot wins depends on which lands first, which is inherent to the local-first model rather than a property of the reconciliation. Write after the connection has settled, or use source transactions, when the outcome has to be deterministic. See [Write Consistency Guarantees](#write-consistency-guarantees) for the full outcome table.

### Connector-internal reconnects do not run the reconciliation

The reconciliation described above runs in the source pump, which covers the initial connection and its retries. Once a connection has succeeded, the connectors handle session loss internally (the OPC UA health loop, the WebSocket monitor loop, the MQTT connection monitor) by reloading initial state without going through the pump's drain and reconcile. A write that was queued for retry before such a reconnect is therefore flushed as-is afterwards, so it overwrites a value the source changed during the outage. That is the opposite of the source-wins rule the pump applies, and the two paths have always disagreed this way.
